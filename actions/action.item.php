    ...
    if (!empty($current_player_items)){
      ...
      foreach ($item_token_list AS $item_key => $item_token){
        ...
        if (!empty($item_token) && in_array($item_token, $current_player_items)){
          ...
          $temp_item = new rpg_ability($this_player, $this_robot, $temp_iteminfo);
          ...
          */
          ...
          $temp_type_info = !empty($temp_item->ability_type) ? rpg_type::get_index_info($temp_item->ability_type) : false;
          $temp_type_info2 = !empty($temp_item->ability_type2) ? rpg_type::get_index_info($temp_item->ability_type2) : false;
          ...
          $temp_item_label .= '<span class="subtext">';
              ...
              $temp_item_label .= ' Damage';
	            ...
	            $temp_item_label .= (!empty($temp_type_info) ? $temp_type_info['type_name'].' ' : 'Neutral ');
	            if (!empty($temp_type_info) && !empty($temp_type_info2)){ $temp_item_label .= ' / '.$temp_type_info2['type_name']; }
          ...
          else { ?><a data-order="<?= $temp_order_counter?>" class="button button_disabled action_ability action_item ability_<?= $temp_item->ability_token ?> ability_type ability_type_<?= $temp_type_class ?> block_<?= $equipped_items_count ?>" type="button"><label class=""><?= $temp_item_sprite['markup'] ?><?= $temp_item_label ?></label></a><?php }
      ...
      if ($equipped_items_count % 8 != 0){
        ...
        for ($i = $equipped_items_count; $i < $temp_last_key; $i++){
          ...
          ?><a class="button action_ability action_item button_disabled block_<?= $i + 1 ?>" type="button">&nbsp;</a><?php
        }
      }
    }
    ?>
