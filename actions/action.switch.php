...
ob_start();
  ...
  ?><div class="main_actions main_actions_hastitle"><span class="main_actions_title" style="<?= !empty($this_player->flags['switch_used_this_turn']) ? 'text-decoration: line-through;' : '' ?>">Select Switch Target <?= $this_switch_disabled ? '(Disabled)' : '' ?></span><?php
  ...
  if (!empty($this_player->player_robots)){
    ...
    foreach ($switch_player_robots AS $robot_key => $switch_robotinfo){
      ...
      if (!empty($switch_robotinfo['robot_token'])){
        ...
        $temp_robot = new rpg_robot($this_player, $switch_robotinfo);
    ...
    if ($num_robots < 8){
      for ($i = $num_robots; $i < 8; $i++){
        ...
        ?><a class="button action_switch button_disabled block_<?= $i + 1 ?>" type="button">&nbsp;</a><?php
      }
    }
  }
  ...
  ?><div class="sub_actions"><a <?= $allow_back_button ? 'data-order="'.$temp_order_counter.'"' : '' ?> class="button action_back <?= !$allow_back_button ? 'button_disabled' : '' ?>" type="button" <?= $allow_back_button ? 'data-panel="battle"' : '' ?>><?= $allow_back_button ? '<label>Back</label>' : '&nbsp;' ?></a></div><?php
