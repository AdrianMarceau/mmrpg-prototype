...
if (in_array('mmrpg_users_unlocked_abilities', $db_tables_list)
    ...
    if (empty($existing_records)){
        ...
        echo('- populating `mmrpg_users_unlocked_abilities` table with user data'.PHP_EOL);
        $user_unlocked_abilities_records = $db->get_array_list("SELECT
            saves.user_id,
        ...
        if (!empty($user_unlocked_abilities_records)){
            foreach ($user_unlocked_abilities_records AS $user_id => $save_data){
                if (empty($save_data['save_values_battle_abilities'])){ continue; }
                $user_unlocked_abilities = json_decode($save_data['save_values_battle_abilities'], true);
                if (!empty($user_unlocked_abilities)){
                    echo('- adding records for user_id '.$user_id.PHP_EOL);
                    //error_log('$user_id = '.print_r($user_id, true));
                    //error_log('$user_unlocked_abilities = '.print_r($user_unlocked_abilities, true));
