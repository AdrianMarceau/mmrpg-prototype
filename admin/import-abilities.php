...
if (!in_array('*', $this_adminaccess)
    ...
    $form_messages[] = array('error', 'You do not have permission to import abilities!');
    redirect_form_action('admin.php?action=home');
}

// Update the page title to show which script this is
$this_page_tabtitle = 'Import Abilities | '.$this_page_tabtitle;
...
$hidden_database_abilities = array_merge($hidden_database_abilities, array('dive-blitzkrieg', 'pharaoh-wave', 'sakugarne-bounce')); // conceptual
...
$temp_pattern_last[] = '/^(item|mobility)-(boost|break|swap)$/i';
$temp_pattern_last[] = '/^(item|mobility)-support$/i';
$temp_pattern_last[] = '/^(item|mobility)-assault$/i';
$temp_pattern_last[] = '/^(item|mobility)-mode$/i';
//$temp_pattern_last[] = '/^(energy|attack|defense|speed|item|repair|weapon|shield|mobility)-(booster|breaker)$/i';
//$temp_pattern_last[] = '/^(energy|attack|defense|speed|item|repair|weapon|shield|mobility)-([a-z0-9]+)$/i';
$temp_stat_types = 'energy|repair|attack|weapon|defense|shield|speed|item|mobility';
