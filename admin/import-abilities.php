...
$hidden_database_abilities = array_merge($hidden_database_abilities, array('ability', 'attachment-defeat', 'action-noweapons', 'action-skip-turn', 'action-charge-energy', 'action-charge-weapons')); // system
...
if (!empty($mmrpg_index['abilities'])){
    foreach ($mmrpg_index['abilities'] AS $ability_token => $ability_data){
        ...
        // Define the flags
        $temp_insert_array['ability_flag_hidden'] = $temp_insert_array['ability_class'] != 'master' || in_array($temp_insert_array['ability_token'], $hidden_database_abilities) || !empty($ability_data['ability_flag_hidden']) ? 1 : 0;
