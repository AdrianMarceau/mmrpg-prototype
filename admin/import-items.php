...
if (!empty($mmrpg_index['items'])){
    foreach ($mmrpg_index['items'] AS $item_token => $item_data){
        ...
        $temp_insert_array['item_functions'] = !empty($item_data['item_functions']) && file_exists(MMRPG_CONFIG_ROOTDIR.'data/'.$item_data['item_functions']) ? $item_data['item_functions'] : 'items/item.php';
        ...
        $temp_insert_array['item_flag_unlockable'] = !empty($temp_insert_array['item_flag_complete']) && !empty($item_data['item_flag_unlockable']) ? 1 : 0;
