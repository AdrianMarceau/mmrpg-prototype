...
if (!in_array('*', $this_adminaccess)
    ...
    $form_messages[] = array('error', 'You do not have permission to import items!');
...
$temp_pattern_last[] = '/^sapling-circuit$/i';
...
$temp_pattern_last[] = '/^master-codes$/i';
...
if (!empty($mmrpg_index['items'])){
    foreach ($mmrpg_index['items'] AS $item_token => $item_data){
        ...
        $temp_insert_array['item_value'] = !empty($item_data['item_value']) ? $item_data['item_value'] : 0;
