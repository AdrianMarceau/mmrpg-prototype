<? ob_start(); ?>
    ...
    $mmrpg_contributors_index = $db->get_array_list("SELECT
        ...
        (CASE WHEN editors.player_image_count IS NOT NULL THEN editors.player_image_count ELSE 0 END) AS user_image_count,
        (CASE WHEN editors2.player_image_count2 IS NOT NULL THEN editors2.player_image_count2 ELSE 0 END) AS user_image_count2
        FROM
        mmrpg_users AS users
        LEFT JOIN mmrpg_roles AS uroles ON uroles.role_id = users.role_id
        LEFT JOIN (SELECT
                player_image_editor AS player_user_id,
                COUNT(player_image_editor) AS player_image_count
                FROM mmrpg_index_players
                GROUP BY player_image_editor) AS editors ON editors.player_user_id = users.user_id
        LEFT JOIN (SELECT
                player_image_editor2 AS player_user_id,
                COUNT(player_image_editor2) AS player_image_count2
                FROM mmrpg_index_players
                GROUP BY player_image_editor2) AS editors2 ON editors2.player_user_id = users.user_id
        WHERE
        users.user_id <> 0
        AND (uroles.role_level > 3
            OR users.user_credit_line <> ''
            OR users.user_credit_text <> ''
            OR editors.player_image_count IS NOT NULL
            OR editors2.player_image_count2 IS NOT NULL)
        ORDER BY
