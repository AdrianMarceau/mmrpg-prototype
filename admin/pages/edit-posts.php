    ...
    */
    ...
    <div class="adminform edit-community edit-posts edit-<?= $this_post_xclass_name ?>" data-baseurl="<?= $this_post_page_baseurl ?>" data-object="post" data-xobject="posts">
        ...
        if ($sub_action == 'editor'
            ...
            $filter_array = array('thread_id' => $post_data['thread_id']);
            ...
            if (!empty($community_thread_posts_array)){
                ...
                foreach ($community_thread_posts_array AS $rel_post_key => $rel_post_data) {
                    $post_num++;
                    $post_id = $rel_post_data['post_id'];
                    $post_body = $format_thread_post_body($rel_post_data['post_body']);
                    $post_edit_url = $this_post_thread_page_baseurl . 'editor/post_id=' . $post_id;
                    $post_view_url = !empty($rel_post_data['post_url']) ? $rel_post_data['post_url'] : '';
                    $post_author_username = htmlspecialchars($rel_post_data['author_name']);
                    $post_author_url = 'admin/edit-users/editor/user_id=' . $rel_post_data['author_id'];
                    ob_start();
                    ?>
                    <li class="<?= $post_id === $post_data['post_id'] ? 'focus ' : ''?><?= !$post_thread_data['thread_published'] || $rel_post_data['post_deleted'] ? 'deleted ' : ''?>">
                        <div class="post-meta">
                            <div class="post-author">
                                By: <a href="<?= $post_author_url ?>" class="author"><?= $post_author_username ?></a>
                            </div>
                            <div class="post-date">
                                On: <ins><?= !empty($rel_post_data['post_date']) ? str_replace('@', 'at', date('Y-m-d @ g:s a', $rel_post_data['post_date'])) : '-' ?></ins>
                            </div>
                            <div class="post-key">
                                <?= $this_post_class_name_uc ?> No. <ins><?= (!$rel_post_data['post_deleted'] ? $post_num : '?').' of '.$community_thread_posts_total ?></ins>
                            </div>
                        </div>
                        <div class="post-actions">
                            <? if ($post_id !== $rel_post_data['post_id']){ ?>
                                <a href="<?= $post_edit_url ?>"><i class="fas fa-pencil-alt"></i><strong>edit in admin</strong></a>
                            <? } ?>
                            <? if ($this_post_class == 'public' && !$rel_post_data['post_deleted'] && !empty($post_view_url)){ ?>
                    ...
                    <form class="form" method="post">
                        ...
                        <div class="editor-panels" data-tabgroup="post">
                            ...
                            <div class="panel active" data-tab="thread">
                                ...
                                <div class="field fullsize">
                                    <strong class="label">
                                        <?= 'Initial '.$this_post_parentclass_name_uc.' w/ ' ?>
                                        <?= $community_thread_posts_total_formatted.' '.($community_thread_posts_total === 1 ? $this_post_class_name_uc : $this_post_xclass_name_uc) ?>
                                    ...
                                    <div class="posts-list <?= $community_thread_posts_count < $community_thread_posts_total ? 'has-load-more' : '' ?>">
                                        <ul>
                                            ...
                                            // Print out the generated markup for sibling posts, if any was generated above
                                            ...
                                            if ($community_thread_posts_count < $community_thread_posts_total){
                                                ...
                                                <li class="load-more load-more-posts" data-num-total="<?= $community_thread_posts_total ?>" data-num-showing="<?= $community_thread_posts_count ?>">
                                                    <a class="more next" data-thread-id="<?= $thread_id ?>" data-load-limit="next">
                                                        Load More <?= $this_post_xclass_name_uc ?>
                                                    </a>
                                                    <a class="more all" data-thread-id="<?= $thread_id ?>" data-load-limit="all">
                                                        Load All <?= $this_post_xclass_name_uc ?>
                                                    </a>
                                                </li>
                                                <?
                                                ob_start();
