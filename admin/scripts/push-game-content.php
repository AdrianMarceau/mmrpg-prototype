...
// Define an array to hold all object tokens and file paths to be published
$publish_tokens = array();
$publish_paths = array();
$publish_paths_bytoken = array();

// If the "all" token was explicitly provided, we're going to publish everything
if ($request_token === 'all'){
    // All all git changes to the list of publish paths
    foreach ($mmrpg_git_changes AS $key => $path){ list($token) = explode('/', $path); if (!in_array($token, $publish_tokens)){ $publish_tokens[] = $token; } }
    foreach ($publish_tokens AS $key => $token){
        $filtered_paths = cms_admin::git_filter_list_by_path($mmrpg_git_changes, $token.'/');
        $publish_paths = array_merge($publish_paths, $filtered_paths);
        $publish_paths_bytoken[$token] = $filtered_paths;
    }
    //debug_echo('publish everything!');
}
// Else we're only going to publish items that match the provided token
else {
    // Only add changes starting with requested token to the list of publish paths
    $publish_tokens = array($request_token);
    $filtered_paths = cms_admin::git_filter_list_by_path($mmrpg_git_changes, $request_token.'/');
    $publish_paths = array_merge($publish_paths, $filtered_paths);
    $publish_paths_bytoken[$request_token] = $filtered_paths;
    //debug_echo('publish only '.$request_token.'!');
}

// Break early if the publish tokens or paths are empty
if (empty($publish_tokens)){ exit_action('error|The publish_tokens were empty (there was nothing to publish)'); }
if (empty($publish_paths)){ exit_action('error|The publish_paths were empty (there was nothing to publish)'); }

//debug_echo('$publish_tokens = '.print_r($publish_tokens, true).'');
//debug_echo('$publish_paths = '.print_r($publish_paths, true).'');
//debug_echo('$publish_paths_bytoken = '.print_r($publish_paths_bytoken, true).'');
...
// Loop through all the publish tokens to undo relevant file and database changes
foreach ($publish_tokens  AS $object_key => $object_token){
    //debug_echo('processing object-token '.$object_token);

    // Collect the file paths to be published
    $object_paths = $publish_paths_bytoken[$object_token];
    ...
    $git_commands .= '&& git pull -s recursive -X theirs --no-edit ';
