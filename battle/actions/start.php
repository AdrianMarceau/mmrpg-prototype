...
if (!empty($_SESSION['ROBOTS_PRELOAD'][$this_battle->battle_token])){
    ...
    foreach ($this_player->values['robots_active'] AS $key => $info){
        ...
        if (!empty($_SESSION['ROBOTS_PRELOAD'][$this_battle->battle_token][$temp_robot_string])){
            ...
            if (!empty($temp_preload_data['robot_abilities'])){ $temp_robot->robot_abilities = $temp_preload_data['robot_abilities']; }
...
// Require the common battle-setup action file
require(MMRPG_CONFIG_ROOTDIR.'battle/actions/action_battlesetup.php');
