...
ob_start();
    ...
    if (!empty($target_player->player_robots)){
        ...
        foreach ($target_player_robots AS $robot_key => $scan_robotinfo){
            ...
            if (!empty($switch_robotinfo['robot_token'])){
                ...
                $robot = rpg_game::get_robot($this_battle, $target_player, $scan_robotinfo);
                $robot_button_markup = rpg_prototype::print_robot_for_battle_menu($robot, 'scan', $robot_key, $button_order);
    ...
    ?><div class="sub_actions"><a data-order="<?=$button_order?>" class="button action_back" type="button" data-panel="battle"><label>Back</label></a></div><?
