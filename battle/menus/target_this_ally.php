...
ob_start();
	...
	if (!empty($this_player->player_robots)){
        ...
        usort($this_player_robots, 'rpg_prototype::sort_robots_for_battle_menu');

        // Loop through each robot and display its target button
        foreach ($this_player_robots AS $robot_key => $target_robotinfo){
            ...
            if (!empty($target_robotinfo['robot_token'])){
                ...
                $robot_button_markup = rpg_prototype::print_robot_for_battle_menu($robot, 'target_this_ally', $robot_key, $button_order);
...
$actions_markup['target_this_ally'] = trim(ob_get_clean());
$actions_markup['target_this_ally'] = preg_replace('#\s+#', ' ', $actions_markup['target_this_ally']);
?>