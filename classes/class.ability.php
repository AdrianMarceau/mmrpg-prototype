...
class rpg_ability extends rpg_object {
  ...
  // Define a function for pulling the full ability index
  public static function get_index($parse_data = false){
    global $this_database;
    $ability_index = $this_database->get_array_list("SELECT * FROM mmrpg_index_abilities WHERE ability_flag_complete = 1 OR ability_token = 'ability';", 'ability_token');
    if (!empty($ability_index)){
      if ($parse_data){ $ability_index = self::parse_index($ability_index); }
  ...
  public static function get_index_custom($ability_tokens = array(), $parse_data = false){
    global $this_database;
    ...
    $ability_index = $this_database->get_array_list("SELECT * FROM mmrpg_index_abilities WHERE ability_token IN ({$ability_tokens_string});", 'ability_token');
    if (!empty($ability_index)){
      if ($parse_data){ $ability_index = self::parse_index($ability_index); }
      return $ability_index;
    } else {
  ...
  public static function get_index_info($ability_token, $parse_data = true){
    global $this_database;
    $ability_index = $this_database->get_array_list("SELECT * FROM mmrpg_index_abilities WHERE ability_token LIKE '{$ability_token}';", 'ability_token');
    if (!empty($ability_index)){
      if ($parse_data){ $ability_index = self::parse_index_info($ability_index); }
      return $ability_index;
    } else {
      return array();
    }
  ...
  public static function parse_index($ability_index){
    foreach ($ability_index AS $token => $info){ $ability_index[$token] = self::parse_index_info($info); }
    return $ability_index;
  }
