...
/**
 * Mega Man RPG Field
 * <p>The object class for all fields in the Mega Man RPG Prototype.</p>
 */
class rpg_field extends rpg_object {

  // Define public field variables
  public $battle = null;
  public $battle_id = 0;
  public $battle_token = '';
  public $field_id = 0;
  ...
  public $field_type2 = '';
  public $field_group = '';
  public $field_multipliers = array();
  public $field_overlays = array();
  public $field_mechas = array();
  public $field_background = '';
  public $field_foreground = '';
  public $field_background_attachments = array();
  public $field_foreground_attachments = array();
  public $field_music = '';
  public $field_base_name = '';
  public $field_base_description = '';
  public $field_base_type = '';
  public $field_base_type2 = '';
  public $field_base_multipliers = array();
  public $field_base_background = '';
  public $field_base_foreground = '';
  public $field_base_background_attachments = array();
  public $field_base_foreground_attachments = array();
  public $field_base_music = '';
  ...
  public function __construct($field_info = array()){
    ...
    if (!isset($this_fieldinfo['field_id'])){ $this_fieldinfo['field_id'] = 0; }
    if (!isset($this_fieldinfo['field_token'])){ $this_fieldinfo['field_token'] = 'field'; }
  ...
  public static function get_field(){
    $this_field = isset($GLOBALS['this_field']) ? $GLOBALS['this_field'] : new rpg_field();
    $this_field->refresh();
    return $this_field;
  }

  /**
   * Manually (re)load field data for this object from the session or index
   * @param integer $field_id
   * @param string $field_token
   * @param array $custom_info (optional)
   * @return bool
   */
  public function field_load($field_id, $field_token, $custom_info = array()){

    // If the field ID was not provided, return false
    if (!isset($field_id)){
      die("field id must be set!\n\$this_fieldinfo\n".print_r($this_fieldinfo, true));
      return false;
    }
    // If the field token was not provided, return false
    if (!isset($field_token)){
      die("field token must be set!\n\$this_fieldinfo\n".print_r($this_fieldinfo, true));
      return false;
    }

    // Collect current field data from the session if available
    if (isset($_SESSION['FIELDS'][$field_id])){
      $this_fieldinfo = $_SESSION['FIELDS'][$field_id];
      if ($this_fieldinfo['field_token'] != $field_token){
        die("field token and ID mismatch {$field_id}:{$field_token}!\n");
        return false;
      }
    }
    // Otherwise, collect field data from the index
    else {
      $this_fieldinfo = rpg_field::get_index_info($field_token);
      if (empty($this_fieldinfo)){
        die("field data could not be loaded for {$field_id}:{$field_token}!\n");
        return false;
      }
    }

    // If the custom data was not empty, merge now
    if (!empty($custom_info)){ $this_fieldinfo = array_merge($this_fieldinfo, $custom_info); }
    ...
    $this->field_type2 = isset($this_fieldinfo['field_type2']) ? $this_fieldinfo['field_type2'] : '';
    ...
    $this->field_base_type2 = isset($this_fieldinfo['field_base_type2']) ? $this_fieldinfo['field_base_type2'] : $this->field_type2;
  ...
  /**
   * Get the formatted database markup for this field object given info and print options
   * @param array $field_info
   * @param array $print_options (optional)
   * @return string
   */
      ...
      <div class="subbody event event_triple event_visible" data-token="<?= $field_info['field_token']?>" style="min-height: 90px; <?= $print_options['layout_style'] == 'website_compact' ? 'margin-bottom: 2px !important;' : '' ?>">
        ...
        <?php if($print_options['show_basics']): ?>
          <h2 class="header header_left field_type_<?= $field_type_token ?>" style="margin-right: 0;">
            ...
            <?php endif; ?>
            <?php if (!empty($field_info['field_type'])): ?>
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td class="right">
                    ...
                    <?php if(!empty($field_info['field_music_name']) && !empty($field_info['field_music_link'])): ?>
                      <?php if(is_array($field_info['field_music_link'])):?>
                        <?php foreach($field_info['field_music_link'] AS $key => $link): ?>
                  ...
                  <td class="right" colspan="3" style="padding: 8px 5px;">
                    ...
                    if (!empty($field_info['field_multipliers'])){
                      ...
                      foreach ($field_info['field_multipliers'] AS $temp_token => $temp_value){
                        $temp_string[] = '<span style="padding: 4px 8px; line-height: 24px; " class="field_multiplier field_type field_type_'.$temp_token.'">'.$mmrpg_types[$temp_token]['type_name'].' x '.number_format($temp_value, 1).'</span>';
                      }
        ...
        <?php if($print_options['show_description'] && !empty($field_info['field_description2'])): ?>
        ...
        <?php if($print_options['show_sprites'] && $field_image_token != 'field'): ?>
  ...
  public static function print_editor_option_markup($field_info){
    ...
    $temp_field_title = rpg_field::print_editor_title_markup($field_info);
  ...
  /**
   * Get the entire field index array with unparsed info
   * @return array
   */
  public static function get_index(){
    global $this_database;
    static $field_index;
    if (empty($field_index)){ $field_index = $this_database->get_array_list("SELECT * FROM mmrpg_index_fields WHERE field_flag_complete = 1;", 'field_token'); }
    if (!empty($field_index)){ return $field_index; }
    else { return array(); }
  }

  /**
   * Get the index info for a specific field via token
   * @param string $field_token
   * @return array
   */
  ...
  /**
   * Parse index info for a given field and expand compressed arrays
   * @param array $field_info
   * @return array
   */
  ...
  /**
   * Update internal variables for this field object
   * @return bool
   */
  public function update_variables(){

    // Update the internal battle field info
    $this->battle->field = $this;
    $this->battle->field_id = $this->field_id;
    $this->battle->field_token = $this->field_token;
  ...
  /**
   * Update the session data for this field object
   * @return bool
   */
  ...
  /**
   * Export an array of the current field object values
   * @return array
   */
