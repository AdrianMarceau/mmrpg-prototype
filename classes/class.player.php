...
class rpg_player extends rpg_object {
  ...
  */
  ...
  /**
   * Get a filtered player index array with parsed info
   * @param array $tokens
   * @return array
   */
  public static function get_index_custom($player_tokens = array()){
    ...
    self::load_player_index();
    $this_index = self::$database_index;
    foreach ($this_index AS $token => $info){
      if (!in_array($token, $player_tokens)){
        unset($this_index[$token]);
      }
    }
    return $this_index;
  }
  ...
  /**
   * Request player info from the global index via player token
   * @param string $player_token
   * @return array
   */
  public static function get_index_info($player_token, $session = true){

    // Collect references to global objects
    $this_database = cms_database::get_database();
    $this_index = self::get_index($session);

    // If the requested player is in the index, return the entry
    if (!empty($this_index[$player_token])){
      // Decode the info and return the array
      $player_info = $this_index[$player_token]; //json_decode($this_index[$player_token], true);
    }
    // Otherwise if the player index doesn't exist at all
    else {
      // Return empty array on failure
      $player_info = array();
    }

    //die('<pre>get_index_info('.$player_token.') = '.print_r($player_info, true).'</pre>');
  ...
  /**
   * Load the player index from the session or cache file and return success
   * @param bool include_session
   * @return bool
   */
  public static function load_player_index($include_session = true){
    ...
    if (!MMRPG_CONFIG_CACHE_INDEXES || !file_exists(MMRPG_CONFIG_PLAYERS_CACHE_PATH)){
      // Start indexing the player data files
      $players_cache_markup = self::index_player_data();
      // Implode the markup into a single string and enclose in PHP tags
      $players_cache_markup = implode('', $players_cache_markup);
      $players_cache_markup = "<?php\n".$players_cache_markup."\n?>";
      // Write the index to a cache file, if caching is enabled
      $players_cache_file = @fopen(MMRPG_CONFIG_PLAYERS_CACHE_PATH, 'w');
    ...
    require_once(MMRPG_CONFIG_PLAYERS_CACHE_PATH);

    //die('<pre>$players_index => '.print_r($players_index, true).'</pre>');

    // Return false if we got nothing from the index
    if (empty($players_index)){ return array(); }

    // Loop through the players and index them after serializing
    foreach ($players_index AS $token => $array){
      $this_index[$token] = $array; //json_encode($array);
    }

    // Additionally, include any dynamic session-based players
    if (!empty($include_session) && !empty($_SESSION['GAME']['values']['player_index'])){
      // The session-based players exist, so merge them with the index
      $this_index = array_merge($this_index, $_SESSION['GAME']['values']['player_index']);
    }
  ...
  /**
   * Generate the player index cache file by scanning the filesystem and return markup
   * @param string $this_path
   * @return string
   */
  public static function index_player_data($this_path = ''){

    // Default the players markup index to an empty array
    $players_cache_markup = array();

    // Open the type data directory for scanning
    $data_players  = opendir(MMRPG_CONFIG_PLAYERS_INDEX_PATH.$this_path);

    // Loop through all the files in the directory
    while (false !== ($filename = readdir($data_players))) {

      // If this is a directory, initiate a recusive scan
      if (is_dir(MMRPG_CONFIG_PLAYERS_INDEX_PATH.$this_path.$filename.'/') && $filename != '.' && $filename != '..'){
        // Collect the markup from the recursive scan
        $append_cache_markup = self::index_player_data($this_path.$filename.'/');
        // If markup was found, append if to the main container
        if (!empty($append_cache_markup)){ $players_cache_markup = array_merge($players_cache_markup, $append_cache_markup); }
      }
      ...
      elseif ($filename != '_index.php' && preg_match('#^[-_a-z0-9]+\.php$#i', $filename)){
        ...
        if (!empty($this_path)){ $this_player_token = trim(str_replace('/', '-', $this_path), '-').'-'.$this_player_token; }

        // Read the file into memory as a string and crop slice out the imporant part
        $this_player_markup = trim(file_get_contents(MMRPG_CONFIG_PLAYERS_INDEX_PATH.$this_path.$filename));
        ...
        $this_player_markup[1] = preg_replace('#\$player = array\(#i', "\$players_index['{$this_player_token}'] = array(\n  'player_token' => '{$this_player_token}',\n  'player_functions' => 'players/{$this_path}{$filename}',", $this_player_markup[1]);
