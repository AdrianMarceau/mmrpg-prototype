...
class rpg_robot extends rpg_object {
  ...
  public static function get_index($parse_data = false){
    global $this_database;
    $robot_index = $this_database->get_array_list("SELECT * FROM mmrpg_index_robots WHERE robot_flag_complete = 1 OR robot_token = 'robot';", 'robot_token');
    if (!empty($robot_index)){
      if ($parse_data){ $robot_index = self::parse_index($robot_index); }
      return $robot_index;
    } else {
      return array();
    }
  ...
  public static function get_index_custom($robot_tokens = array(), $parse_data = false){
    global $this_database;
    ...
    $robot_index = $this_database->get_array_list("SELECT * FROM mmrpg_index_robots WHERE robot_token IN ({$robot_tokens_string});", 'robot_token');
    if (!empty($robot_index)){
      if ($parse_data){ $robot_index = self::parse_index($robot_index); }
      return $robot_index;
    } else {
  ...
  public static function get_index_info($robot_token, $parse_data = true){
    global $this_database;
    $robot_index = $this_database->get_array_list("SELECT * FROM mmrpg_index_robots WHERE robot_token LIKE '{$robot_token}';", 'robot_token');
    if (!empty($robot_index)){
      if ($parse_data){ $robot_index = self::parse_index_info($robot_index); }
      return $robot_index;
    } else {
      return array();
    }
  ...
  public static function parse_index($robot_index){
    foreach ($robot_index AS $token => $info){ $robot_index[$token] = self::parse_index_info($info); }
    return $robot_index;
  }
