<?php
// Define the rpg_website() class
class rpg_website {

  // Define the constructor class
  public function rpg_website(){ }
  ...
  public static function formatting_decode($string){
    ...
    if (empty($mmrpg_formatting_array)){
      ...
      $mmrpg_types_array = array_keys($mmrpg_types);
  ...
  public static function formatting_encode($string){
    // Pull in the global index formatting variables
  ...
  public static function formatting_help(){

    // Collect the formatting file contents into a variable
    $this_formatting = file_get_contents(MMRPG_CONFIG_ROOTDIR.'includes/include.formatting.txt');
    $this_formatting = nl2br(rpg_website::formatting_decode($this_formatting));
  ...
  public static function number_suffix($value, $concatenate = true, $superscript = false){
  ...
  public static function print_online($this_leaderboard_online_players = array(), $filter_userids = array()){
    if (empty($this_leaderboard_online_players)){ $this_leaderboard_online_players = rpg_prototype::leaderboard_online(); }
    ob_start();
  ...
  public static function sessions_active($session_href = '', $session_timeout = 3, $strict_filtering = false){
    // Import required global variables
    global $this_database, $this_userid;
    ...
    if (!$strict_filtering){
      ...
      $active_sessions = $this_database->get_array_list("SELECT DISTINCT user_id, session_href FROM mmrpg_sessions WHERE session_access >= {$min_time} {$inner_href_query} ORDER BY session_access DESC", 'user_id');
    }
    // Otherwise, we have to excluce users who have since visited other pages
    else {
      // Collect any sessions that are active and match the query
      $active_sessions = $this_database->get_array_list("SELECT DISTINCT user_id, session_href FROM mmrpg_sessions WHERE session_access >= {$min_time} ORDER BY session_access ASC", 'user_id');
  ...
  public static function session_update($session_href){
    // Import required global variables
    global $this_database, $this_userid;
    // Collect the session ID from the system
    $session_key = session_id();
    // Attempt to collect the current database row if it exists
    $temp_session = $this_database->get_array("SELECT * FROM mmrpg_sessions WHERE user_id = '{$this_userid}' AND session_key = '{$session_key}' AND session_href = '{$session_href}' LIMIT 1");
    ...
    if (!empty($temp_session['session_id'])){
      ...
      $this_database->update('mmrpg_sessions', $update_array, array('session_id' => $temp_session['session_id']));
    }
    // Else if first visit to this page during this session, insert it
      ...
      $this_database->insert('mmrpg_sessions', $insert_array);
    }
  ...
  public static function community_index(){
    global $this_database;
    ...
    if (false && !empty($_SESSION['COMMUNITY']['categories'])){
      ...
      $this_categories_index = $this_database->get_array_list($this_categories_query, 'category_token');
  ...
  public static function community_thread_linkblock($this_thread_info, $this_category_info = array(), $display_style = 'full'){
    ...
    <div id="thread-<?= $temp_thread_id ?>" data-group="<?= $temp_date_group ?>" data-key="<?= $this_thread_key ?>"  class="subbody thread_subbody thread_subbody_small <?= !empty($this_thread_info['thread_locked']) ? 'locked' : '' ?> <?= $display_style == 'compact' ? 'thread_subbody_compact' : '' ?> thread_right field_type_<?= !empty($this_thread_info['thread_colour']) ? $this_thread_info['thread_colour'] : 'none' ?>" style="text-align: left; margin: 2px 0;">
      ...
      <div class="text thread_linkblock thread_linkblock_<?= $this_thread_info['thread_target'] != 0 && $this_thread_info['user_id'] != $this_userinfo['user_id'] ? 'right' : 'left' ?>">
        ...
        <div class="info">
          ...
          <?php if($this_category_info['category_token'] == 'search' || $this_category_info['category_token'] == 'leaderboard'): ?>
      ...
      <?php if(empty($this_post_info['post_deleted'])): ?>
        <div class="userblock player_type_<?= $temp_avatar_colour ?>">
          ...
          <div class="<?= $temp_avatar_class ?> avatar_userimage" style="">
            ...
            <?php if($temp_is_contributor): ?><div class="<?= $temp_item_class ?>" style="background-image: url(<?= $temp_item_path ?>); position: absolute; top: -10px; <?= $this_post_float ?>: -14px;" title="<?= $temp_item_title ?>"><?= $temp_item_title ?></div><?php endif; ?>
            <div class="<?= $temp_sprite_class ?>" style="background-image: url(<?= $temp_sprite_path ?>);"><?= $temp_post_author ?></div>
          </div>

          <?php $temp_stat = !empty($this_user_countindex[$this_post_info['user_id']]['board_points']) ? $this_user_countindex[$this_post_info['user_id']]['board_points'] : 0; ?>
          <div class="counter points_counter"><?= number_format($temp_stat, 0, '.', ',').' BP' ?></div>
          <div class="counter community_counters">
            <?php $temp_stat = !empty($this_user_countindex[$this_post_info['user_id']]['thread_count']) ? $this_user_countindex[$this_post_info['user_id']]['thread_count'] : 0; ?>
            <span class="thread_counter"><?= $temp_stat.' TP' ?></span> <span class="pipe">|</span>
            <?php $temp_stat = !empty($this_user_countindex[$this_post_info['user_id']]['post_count']) ? $this_user_countindex[$this_post_info['user_id']]['post_count'] : 0; ?>
        ...
        <div class="postblock">
          ...
          <?php if (!empty($this_post_info['post_is_thread'])): ?>
          ...
          <div class="published" title="<?= $temp_post_author.' on '.str_replace(' ', '&nbsp;', $temp_post_date) ?>">
            <strong>
              ...
              <?php if(empty($this_post_info['post_is_thread']) && ($this_category_info['category_token'] == 'search' || $this_category_info['category_token'] == 'leaderboard')): ?>
            ...
            <?php if (true || $this_category_info['category_token'] != 'search' && $this_category_info['category_token'] != 'leaderboard'): ?>
            ...
            <?php if(!$temp_post_guest && (COMMUNITY_VIEW_MODERATOR || $this_userinfo['user_id'] == $this_post_info['user_id'])): ?>
          ...
          <div class="bodytext">
            <?= rpg_website::formatting_decode($temp_post_body) ?>
          </div>

        </div>
        <?php if($this_userid != MMRPG_SETTINGS_GUEST_ID && empty($this_thread_info['thread_locked']) && $community_battle_points > MMRPG_SETTINGS_POST_MINPOINTS && $this_category_info['category_token'] != 'personal'): ?>
  ...
  // Define a function for getting possessive words based on gender
  public static function gendered_subject($gender){
  ...
  public static function gendered_object($gender){
  ...
  public static function get_cached_markup($filename){
  ...
  public static function save_cached_markup($filename, $markup){
    // If the filename was empty, return false
    if (empty($filename)){ return false; }
    // Define the base path for all cache files
    $temp_path_base = MMRPG_CONFIG_ROOTDIR.'data/cache/';
