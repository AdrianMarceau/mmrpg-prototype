...
class cms_database {
    ...
    public function query($query_string, &$affected_rows = 0){
        ...
        if ($this->MYSQL_RESULT === false){
            ...
            if (!MMRPG_CONFIG_IS_LIVE || MMRPG_CONFIG_DEBUG_MODE){ $this->critical_error("[[cms_database::query]] : Unable to run the requested query. ".mysqli_errno($this->LINK).". The query was &laquo;<pre>".htmlentities($query_string, ENT_QUOTES, 'UTF-8')."</pre>&raquo;. <br /> {$backtrace}"); }
