...
class rpg_ability_damage extends rpg_damage {
    ...
    public static function trigger_robot_damage($this_robot, $target_robot, $this_ability, $damage_amount, $trigger_disabled = true, $trigger_options = array()){
        ...
        // DEBUG
        //$this_robot->battle->events_create(false, false, 'DEBUG_'.__LINE__, $this_ability->ability_token.' | this('.$this_robot->robot_id.') vs target('.$target_robot->robot_id.') | damage_start_amount |<br /> '.'amount:'.$this_ability->ability_results['this_amount'].' | '.'percent:'.($this_ability->damage_options['damage_percent'] ? 'true' : 'false').' | '.'kind:'.$this_ability->damage_options['damage_kind'].' | type1:'.(!empty($this_ability->damage_options['damage_type']) ? $this_ability->damage_options['damage_type'] : 'none').' | type2:'.(!empty($this_ability->damage_options['damage_type2']) ? $this_ability->damage_options['damage_type2'] : 'none').'');

        // DEBUG
        foreach ($trigger_options AS $key => $value){ $debug .= $key.'='.($value === true ? 'true' : ($value === false ? 'false' : $value)).'; '; }
        //$this_robot->battle->events_create(false, false, 'DEBUG_'.__LINE__, $this_ability->ability_token.' : damage_trigger_options : '.$debug);
        ...
        if ($trigger_options['apply_modifiers'] != false){
            ...
            if ($trigger_options['apply_position_modifiers'] != false){
                ...
                if ($this_robot->robot_position != 'active'){
                    ...
                    // DEBUG
                    //$this_robot->battle->events_create(false, false, 'DEBUG_'.__LINE__, $this_ability->ability_token.' | position_modifier_damage | '.$damage_amount.' = round('.$damage_amount.' * '.$temp_damage_resistor.') = '.$new_damage_amount.'');
        ...
        if ($trigger_options['apply_field_modifiers'] != false && $this_ability->damage_options['damage_modifiers'] && !empty($this_robot->field->field_multipliers)){
            ...
            if (isset($field_multipliers['damage'])){
                ...
                // DEBUG
                //$this_robot->battle->events_create(false, false, 'DEBUG_'.__LINE__, $this_ability->ability_token.' | field_multiplier_damage | '.$damage_amount.' = round('.$damage_amount.' * '.$field_multipliers['damage'].') = '.$new_damage_amount.'');
                $damage_amount = $new_damage_amount;
            }
            ...
            foreach ($field_multipliers AS $temp_type => $temp_multiplier){
                ...
                if ($temp_ability_damage_type == $temp_type){
                    ...
                    // DEBUG
                    //$this_robot->battle->events_create(false, false, 'DEBUG_'.__LINE__, $this_ability->ability_token.' | field_multiplier_'.$temp_type.' | '.$damage_amount.' = round('.$damage_amount.' * '.$temp_multiplier.') = '.$new_damage_amount.'');
                    $damage_amount = $new_damage_amount;
                }
        ...
        if ($trigger_options['apply_starforce_modifiers'] != false && $this_ability->damage_options['damage_modifiers'] && !empty($target_robot->player->player_starforce) && $this_robot->robot_id != $target_robot->robot_id){
            ...
            if (!empty($temp_ability_damage_type) && !empty($target_starforce_modified[$temp_ability_damage_type])){
                ...
                $new_damage_amount = round($damage_amount * $temp_multiplier);
                // DEBUG
                //$this_robot->battle->events_create(false, false, 'DEBUG_'.__LINE__, $this_ability->ability_token.' | starforce_multiplier_'.$temp_ability_damage_type.' | force:'.$target_starforce[$temp_ability_damage_type].' vs resist:'.$this_starforce[$temp_ability_damage_type].' = '.($target_starforce_modified[$temp_ability_damage_type] * 10).'% boost | '.$damage_amount.' = round('.$damage_amount.' * '.$temp_multiplier.') = '.$new_damage_amount.'');
                $damage_amount = $new_damage_amount;
            } elseif (!empty($temp_ability_damage_type) && isset($target_starforce_modified[$temp_ability_damage_type])){
                // DEBUG
                //$this_robot->battle->events_create(false, false, 'DEBUG_'.__LINE__, $this_ability->ability_token.' | starforce_multiplier_'.$temp_ability_damage_type.' | force:'.$target_starforce[$temp_ability_damage_type].' vs resist:'.$this_starforce[$temp_ability_damage_type].' = no boost');
            }
            ...
            if (!empty($temp_ability_damage_type2) && !empty($target_starforce_modified[$temp_ability_damage_type2])){
                ...
                $new_damage_amount = round($damage_amount * $temp_multiplier);
                // DEBUG
                //$this_robot->battle->events_create(false, false, 'DEBUG_'.__LINE__, $this_ability->ability_token.' | starforce_multiplier_'.$temp_ability_damage_type2.' | force:'.$target_starforce[$temp_ability_damage_type2].' vs resist:'.$this_starforce[$temp_ability_damage_type2].' = '.($target_starforce_modified[$temp_ability_damage_type2] * 10).'% boost | '.$damage_amount.' = round('.$damage_amount.' * '.$temp_multiplier.') = '.$new_damage_amount.'');
                $damage_amount = $new_damage_amount;
            } elseif (!empty($temp_ability_damage_type2) && isset($target_starforce_modified[$temp_ability_damage_type2])){
                // DEBUG
                //$this_robot->battle->events_create(false, false, 'DEBUG_'.__LINE__, $this_ability->ability_token.' | starforce_multiplier_'.$temp_ability_damage_type2.' | force:'.$target_starforce[$temp_ability_damage_type2].' vs resist:'.$this_starforce[$temp_ability_damage_type2].' = no boost');
            }
            ...
            if ($trigger_options['apply_stat_modifiers'] != false && !$this_ability->damage_options['damage_percent']){
                ...
                if ($this_ability->damage_options['damage_kind'] == 'energy' && ($this_robot->robot_id != $target_robot->robot_id || $trigger_options['referred_damage'])){
                    ...
                    // Otherwise if both robots have normal stats, calculate the new amount normally
                        ...
                        // DEBUG
                        //$this_robot->battle->events_create(false, false, 'DEBUG_'.__LINE__, $this_ability->ability_token.' | normal_damage | A:'.$target_robot->robot_attack.' D:'.$this_robot->robot_defense.' | '.$this_ability->ability_results['this_amount'].' = round('.$this_ability->ability_results['this_amount'].' * ('.$target_robot->robot_attack.' / '.$this_robot->robot_defense.')) = '.$temp_new_amount.'');
                        // Update the amount with the new calculation
                        $this_ability->ability_results['this_amount'] = $temp_new_amount;
                    }
                ...
                if ($this_robot->battle->critical_chance($this_ability->damage_options['critical_rate'])){
                    ...
                    // DEBUG
                    //$this_robot->battle->events_create(false, false, 'DEBUG_'.__LINE__, $this_ability->ability_token.' | flag_critical | x '.$this_ability->damage_options['critical_multiplier'].' = '.$this_ability->ability_results['this_amount'].'');
                } else {
            ...
            if ($trigger_options['apply_type_modifiers'] != false){
                ...
                if ($this_ability->ability_results['flag_weakness']){
                    ...
                    for ($i = 1; $i <= $loop_count; $i++){
                        ...
                        // DEBUG
                        //$this_robot->battle->events_create(false, false, 'DEBUG_'.__LINE__, $this_ability->ability_token.' | flag_weakness ('.$i.'/'.$loop_count.') | '.$this_ability->ability_results['this_amount'].' = round('.$this_ability->ability_results['this_amount'].' * '.$this_ability->damage_options['weakness_multiplier'].') = '.$temp_new_amount.'');
                        $this_ability->ability_results['this_amount'] = $temp_new_amount;
                    }
                }
                ...
                if ($this_ability->ability_results['flag_resistance']){
                    ...
                    for ($i = 1; $i <= $loop_count; $i++){
                        ...
                        // DEBUG
                        //$this_robot->battle->events_create(false, false, 'DEBUG_'.__LINE__, $this_ability->ability_token.' | flag_resistance ('.$i.'/'.$loop_count.') | '.$this_ability->ability_results['this_amount'].' = round('.$this_ability->ability_results['this_amount'].' * '.$this_ability->damage_options['resistance_multiplier'].') = '.$temp_new_amount.'');
                        $this_ability->ability_results['this_amount'] = $temp_new_amount;
                    }
                }
                ...
                if ($this_ability->ability_results['flag_immunity']){
                    ...
                    for ($i = 1; $i <= $loop_count; $i++){
                        ...
                        // DEBUG
                        //$this_robot->battle->events_create(false, false, 'DEBUG_'.__LINE__, $this_ability->ability_token.' | flag_immunity ('.$i.'/'.$loop_count.') | '.$this_ability->ability_results['this_amount'].' = round('.$this_ability->ability_results['this_amount'].' * '.$this_ability->damage_options['immunity_multiplier'].') = '.$temp_new_amount.'');
                        $this_ability->ability_results['this_amount'] = $temp_new_amount;
                    }
                }
        ...
        if ($this_robot->robot_energy <= 0 && $this_robot_energy_start_max){
            // DEBUG
            //$this_robot->battle->events_create(false, false, 'DEBUG_'.__LINE__, $this_ability->ability_token.' | damage_result_OHKO! | Start:'.$this_robot_energy_start.' '.($this_robot_energy_start_max ? '(MAX!)' : '-').' | Finish:'.$this_robot->robot_energy);
