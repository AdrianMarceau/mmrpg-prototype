...
class rpg_item_recovery extends rpg_recovery {
    ...
    public static function trigger_robot_recovery($this_robot, $target_robot, $this_item, $recovery_amount, $trigger_disabled = true, $trigger_options = array()){
        global $db;
        //error_log('rpg_item_recovery::trigger_robot_recovery() from '.(!empty($this_item) ? $this_item->item_token : 'unknown'));
        ...
        rpg_canvas::apply_camera_action_flags($event_options, $this_robot, $this_item, 'recovery');
        ...
        if (!empty($anti_recovery_robots)){
            ...
            $this_battle->events_create(
                $this_robot,
                false,
                $this_item->recovery_options['recovery_header'],
            ...
            if ($trigger_options['apply_modifiers'] != false
                ...
                if ($apply_origin_attachment_modifiers
                    ...
                    foreach ($target_robot_attachments AS $temp_token => $temp_info){
                        ...
                        if (!empty($temp_item_recovery_type)){
                            // If this robot's attachment has a recovery breaker value set
                            if ($apply_origin_attachment_recovery_breakers
                                && isset($temp_info['attachment_recovery_breaker_'.$temp_item_recovery_type])){
                                // Apply the recovery breaker multiplier to the current recovery amount
                                $temp_new_amount = ($this_item->item_results['this_amount'] * $temp_info['attachment_recovery_breaker_'.$temp_item_recovery_type]);
                                $this_battle->events_debug(__FILE__, __LINE__, 'item_'.$this_item->item_token.' vs. '.$temp_token_debug.' <br /> attachment_recovery_breaker_'.$temp_item_recovery_type.' | '.$this_item->item_results['this_amount'].' = ('.$this_item->item_results['this_amount'].' * '.$temp_info['attachment_recovery_breaker_'.$temp_item_recovery_type].') = '.$temp_new_amount.'');
                                $this_item->item_results['this_amount'] = $temp_new_amount;
                            }
                            // If this robot's attachment has a recovery booster value set
                            if ($apply_origin_attachment_recovery_boosters
                                && isset($temp_info['attachment_recovery_booster_'.$temp_item_recovery_type])){
                                // Apply the recovery booster multiplier to the current recovery amount
                                $temp_new_amount = ($this_item->item_results['this_amount'] * $temp_info['attachment_recovery_booster_'.$temp_item_recovery_type]);
                                $this_battle->events_debug(__FILE__, __LINE__, 'item_'.$this_item->item_token.' vs. '.$temp_token_debug.' <br /> attachment_recovery_booster_'.$temp_item_recovery_type.' | '.$this_item->item_results['this_amount'].' = ('.$this_item->item_results['this_amount'].' * '.$temp_info['attachment_recovery_booster_'.$temp_item_recovery_type].') = '.$temp_new_amount.'');
                                $this_item->item_results['this_amount'] = $temp_new_amount;
                            }
                            // If this robot's attachment has a recovery breaker value set
                            if ($apply_origin_attachment_recovery_breakers
                                && isset($temp_info['attachment_recovery_output_breaker_'.$temp_item_recovery_type])){
                                // Apply the recovery breaker multiplier to the current recovery amount
                                $temp_new_amount = ($this_item->item_results['this_amount'] * $temp_info['attachment_recovery_output_breaker_'.$temp_item_recovery_type]);
                                $this_battle->events_debug(__FILE__, __LINE__, 'item_'.$this_item->item_token.' vs. '.$temp_token_debug.' <br /> attachment_recovery_output_breaker_'.$temp_item_recovery_type.' | '.$this_item->item_results['this_amount'].' = ('.$this_item->item_results['this_amount'].' * '.$temp_info['attachment_recovery_output_breaker_'.$temp_item_recovery_type].') = '.$temp_new_amount.'');
                                $this_item->item_results['this_amount'] = $temp_new_amount;
                            }
                            // If this robot's attachment has a recovery booster value set
                            if ($apply_origin_attachment_recovery_boosters
                                && isset($temp_info['attachment_recovery_output_booster_'.$temp_item_recovery_type])){
                                // Apply the recovery booster multiplier to the current recovery amount
                                $temp_new_amount = ($this_item->item_results['this_amount'] * $temp_info['attachment_recovery_output_booster_'.$temp_item_recovery_type]);
                                $this_battle->events_debug(__FILE__, __LINE__, 'item_'.$this_item->item_token.' vs. '.$temp_token_debug.' <br /> attachment_recovery_output_booster_'.$temp_item_recovery_type.' | '.$this_item->item_results['this_amount'].' = ('.$this_item->item_results['this_amount'].' * '.$temp_info['attachment_recovery_output_booster_'.$temp_item_recovery_type].') = '.$temp_new_amount.'');
                                $this_item->item_results['this_amount'] = $temp_new_amount;
                            }
                        }
                        // Next check to see if any breakers or boosters for either of this item's types
                        if (!empty($temp_item_recovery_type2)){
                            // If this robot's attachment has a recovery breaker value set
                            if ($apply_origin_attachment_recovery_breakers
                                && isset($temp_info['attachment_recovery_breaker_'.$temp_item_recovery_type2])){
                                // Apply the recovery breaker multiplier to the current recovery amount
                                $temp_new_amount = ($this_item->item_results['this_amount'] * $temp_info['attachment_recovery_breaker_'.$temp_item_recovery_type2]);
                                $this_battle->events_debug(__FILE__, __LINE__, 'item_'.$this_item->item_token.' vs. '.$temp_token_debug.' <br /> attachment_recovery_breaker_'.$temp_item_recovery_type2.' | '.$this_item->item_results['this_amount'].' = ('.$this_item->item_results['this_amount'].' * '.$temp_info['attachment_recovery_breaker_'.$temp_item_recovery_type2].') = '.$temp_new_amount.'');
                                $this_item->item_results['this_amount'] = $temp_new_amount;
                            }
                            // If this robot's attachment has a recovery booster value set
                            if ($apply_origin_attachment_recovery_boosters
                                && isset($temp_info['attachment_recovery_booster_'.$temp_item_recovery_type2])){
                                // Apply the recovery booster multiplier to the current recovery amount
                                $temp_new_amount = ($this_item->item_results['this_amount'] * $temp_info['attachment_recovery_booster_'.$temp_item_recovery_type2]);
                                $this_battle->events_debug(__FILE__, __LINE__, 'item_'.$this_item->item_token.' vs. '.$temp_token_debug.' <br /> attachment_recovery_booster_'.$temp_item_recovery_type2.' | '.$this_item->item_results['this_amount'].' = ('.$this_item->item_results['this_amount'].' * '.$temp_info['attachment_recovery_booster_'.$temp_item_recovery_type2].') = '.$temp_new_amount.'');
                                $this_item->item_results['this_amount'] = $temp_new_amount;
                            }
                            // If this robot's attachment has a recovery breaker value set
                            if ($apply_origin_attachment_recovery_breakers
                                && isset($temp_info['attachment_recovery_output_breaker_'.$temp_item_recovery_type2])){
                                // Apply the recovery breaker multiplier to the current recovery amount
                                $temp_new_amount = ($this_item->item_results['this_amount'] * $temp_info['attachment_recovery_output_breaker_'.$temp_item_recovery_type2]);
                                $this_battle->events_debug(__FILE__, __LINE__, 'item_'.$this_item->item_token.' vs. '.$temp_token_debug.' <br /> attachment_recovery_output_breaker_'.$temp_item_recovery_type2.' | '.$this_item->item_results['this_amount'].' = ('.$this_item->item_results['this_amount'].' * '.$temp_info['attachment_recovery_output_breaker_'.$temp_item_recovery_type2].') = '.$temp_new_amount.'');
                                $this_item->item_results['this_amount'] = $temp_new_amount;
                            }
                            // If this robot's attachment has a recovery booster value set
                            if ($apply_origin_attachment_recovery_boosters
                                && isset($temp_info['attachment_recovery_output_booster_'.$temp_item_recovery_type2])){
                                // Apply the recovery booster multiplier to the current recovery amount
                                $temp_new_amount = ($this_item->item_results['this_amount'] * $temp_info['attachment_recovery_output_booster_'.$temp_item_recovery_type2]);
                                $this_battle->events_debug(__FILE__, __LINE__, 'item_'.$this_item->item_token.' vs. '.$temp_token_debug.' <br /> attachment_recovery_output_booster_'.$temp_item_recovery_type2.' | '.$this_item->item_results['this_amount'].' = ('.$this_item->item_results['this_amount'].' * '.$temp_info['attachment_recovery_output_booster_'.$temp_item_recovery_type2].') = '.$temp_new_amount.'');
                                $this_item->item_results['this_amount'] = $temp_new_amount;
                            }
                        }
                ...
                if ($apply_target_attachment_modifiers
                    ...
                    foreach ($this_robot_attachments AS $temp_token => $temp_info){
                        ...
                        if (!empty($temp_item_recovery_type)){
                            ...
                            // If this robot's attachment has a recovery breaker value set
                            if ($apply_target_attachment_recovery_breakers
                                && isset($temp_info['attachment_recovery_input_breaker_'.$temp_item_recovery_type])){
                                // Apply the recovery breaker multiplier to the current recovery amount
                                $temp_new_amount = ($this_item->item_results['this_amount'] * $temp_info['attachment_recovery_input_breaker_'.$temp_item_recovery_type]);
                                $this_battle->events_debug(__FILE__, __LINE__, 'item_'.$this_item->item_token.' vs. '.$temp_token_debug.' <br /> attachment_recovery_input_breaker_'.$temp_item_recovery_type.' | '.$this_item->item_results['this_amount'].' = ('.$this_item->item_results['this_amount'].' * '.$temp_info['attachment_recovery_input_breaker_'.$temp_item_recovery_type].') = '.$temp_new_amount.'');
                                $this_item->item_results['this_amount'] = $temp_new_amount;
                            }
                            // If this robot's attachment has a recovery booster value set
                            if ($apply_target_attachment_recovery_boosters
                                && isset($temp_info['attachment_recovery_input_booster_'.$temp_item_recovery_type])){
                                // Apply the recovery booster multiplier to the current recovery amount
                                $temp_new_amount = ($this_item->item_results['this_amount'] * $temp_info['attachment_recovery_input_booster_'.$temp_item_recovery_type]);
                                $this_battle->events_debug(__FILE__, __LINE__, 'item_'.$this_item->item_token.' vs. '.$temp_token_debug.' <br /> attachment_recovery_input_booster_'.$temp_item_recovery_type.' | '.$this_item->item_results['this_amount'].' = ('.$this_item->item_results['this_amount'].' * '.$temp_info['attachment_recovery_input_booster_'.$temp_item_recovery_type].') = '.$temp_new_amount.'');
                                $this_item->item_results['this_amount'] = $temp_new_amount;
                            }
                        }
                        // Next check to see if any breakers or boosters for either of this item's types
                        if (!empty($temp_item_recovery_type2)){
                            ...
                            // If this robot's attachment has a recovery breaker value set
                            if ($apply_target_attachment_recovery_breakers
                                && isset($temp_info['attachment_recovery_input_breaker_'.$temp_item_recovery_type2])){
                                // Apply the recovery breaker multiplier to the current recovery amount
                                $temp_new_amount = ($this_item->item_results['this_amount'] * $temp_info['attachment_recovery_input_breaker_'.$temp_item_recovery_type2]);
                                $this_battle->events_debug(__FILE__, __LINE__, 'item_'.$this_item->item_token.' vs. '.$temp_token_debug.' <br /> attachment_recovery_input_breaker_'.$temp_item_recovery_type2.' | '.$this_item->item_results['this_amount'].' = ('.$this_item->item_results['this_amount'].' * '.$temp_info['attachment_recovery_input_breaker_'.$temp_item_recovery_type2].') = '.$temp_new_amount.'');
                                $this_item->item_results['this_amount'] = $temp_new_amount;
                            }
                            // If this robot's attachment has a recovery booster value set
                            if ($apply_target_attachment_recovery_boosters
                                && isset($temp_info['attachment_recovery_input_booster_'.$temp_item_recovery_type2])){
                                // Apply the recovery booster multiplier to the current recovery amount
                                $temp_new_amount = ($this_item->item_results['this_amount'] * $temp_info['attachment_recovery_input_booster_'.$temp_item_recovery_type2]);
                                $this_battle->events_debug(__FILE__, __LINE__, 'item_'.$this_item->item_token.' vs. '.$temp_token_debug.' <br /> attachment_recovery_input_booster_'.$temp_item_recovery_type2.' | '.$this_item->item_results['this_amount'].' = ('.$this_item->item_results['this_amount'].' * '.$temp_info['attachment_recovery_input_booster_'.$temp_item_recovery_type2].') = '.$temp_new_amount.'');
                                $this_item->item_results['this_amount'] = $temp_new_amount;
                            }
                        }
                        ...
                        if (!empty($this_item->item_type)
                            ...
                            if (!empty($attachment_info['attachment_weaknesses'])
                                ...
                                if ($attachment_destroy_info !== false){
                                    ...
                                    if (isset($attachment_info['item_token'])){ $temp_attachment = rpg_game::get_item($this_robot->battle, $this_robot->player, $this_robot, array('item_token' => $attachment_info['item_token'])); }
