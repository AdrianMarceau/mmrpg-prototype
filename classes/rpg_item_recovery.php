    ...
    public static function trigger_robot_recovery($this_robot, $target_robot, $this_item, $recovery_amount, $trigger_disabled = true, $trigger_options = array()){
        ...
        if ($trigger_options['apply_modifiers'] != false){
            ...
            if ($trigger_options['apply_position_modifiers'] != false){
                ...
                if ($this_robot->robot_position != 'active'){
                    ...
                    $this_battle->events_debug(__FILE__, __LINE__, $this_item->item_token.' | position_modifier_recovery | '.$recovery_amount.' = round('.$recovery_amount.' * '.$temp_recovery_resistor.') = '.$new_recovery_amount.'');
        ...
        if ($trigger_options['apply_field_modifiers'] != false && $this_item->recovery_options['recovery_modifiers'] && !empty($this_robot->field->field_multipliers)){
            ...
            if (isset($field_multipliers['recovery'])){
                ...
                $this_battle->events_debug(__FILE__, __LINE__, $this_item->item_token.' | field_multiplier_recovery | '.$recovery_amount.' = round('.$recovery_amount.' * '.$field_multipliers['recovery'].') = '.$new_recovery_amount.'');
                $recovery_amount = $new_recovery_amount;
            }
            ...
            foreach ($field_multipliers AS $temp_type => $temp_multiplier){
                ...
                if ($temp_item_recovery_type == $temp_type){
                    ...
                    $this_battle->events_debug(__FILE__, __LINE__, $this_item->item_token.' | field_multiplier_'.$temp_type.' | '.$recovery_amount.' = round('.$recovery_amount.' * '.$temp_multiplier.') = '.$new_recovery_amount.'');
                    $recovery_amount = $new_recovery_amount;
                }
        ...
        if ($trigger_options['apply_starforce_modifiers'] != false && $this_item->recovery_options['recovery_modifiers'] && !empty($target_robot->player->player_starforce) && $this_robot->robot_id != $target_robot->robot_id){
            ...
            if (!empty($temp_item_recovery_type) && !empty($target_starforce_modified[$temp_item_recovery_type])){
                ...
                $new_recovery_amount = round($recovery_amount * $temp_multiplier);
                $this_battle->events_debug(__FILE__, __LINE__, $this_item->item_token.' | starforce_multiplier_'.$temp_item_recovery_type.' | force:'.$target_starforce[$temp_item_recovery_type].' vs resist:'.$this_starforce[$temp_item_recovery_type].' = '.($target_starforce_modified[$temp_item_recovery_type] * 10).'% boost | '.$recovery_amount.' = round('.$recovery_amount.' * '.$temp_multiplier.') = '.$new_recovery_amount.'');
                $recovery_amount = $new_recovery_amount;
            } elseif (!empty($temp_item_recovery_type) && isset($target_starforce_modified[$temp_item_recovery_type])){
                $this_battle->events_debug(__FILE__, __LINE__, $this_item->item_token.' | starforce_multiplier_'.$temp_item_recovery_type.' | force:'.$target_starforce[$temp_item_recovery_type].' vs resist:'.$this_starforce[$temp_item_recovery_type].' = no boost');
            }
            ...
            if (!empty($temp_item_recovery_type2) && !empty($target_starforce_modified[$temp_item_recovery_type2])){
                ...
                $new_recovery_amount = round($recovery_amount * $temp_multiplier);
                $this_battle->events_debug(__FILE__, __LINE__, $this_item->item_token.' | starforce_multiplier_'.$temp_item_recovery_type2.' | force:'.$target_starforce[$temp_item_recovery_type2].' vs resist:'.$this_starforce[$temp_item_recovery_type2].' = '.($target_starforce_modified[$temp_item_recovery_type2] * 10).'% boost | '.$recovery_amount.' = round('.$recovery_amount.' * '.$temp_multiplier.') = '.$new_recovery_amount.'');
                $recovery_amount = $new_recovery_amount;
            } elseif (!empty($temp_item_recovery_type2) && isset($target_starforce_modified[$temp_item_recovery_type2])){
                $this_battle->events_debug(__FILE__, __LINE__, $this_item->item_token.' | starforce_multiplier_'.$temp_item_recovery_type2.' | force:'.$target_starforce[$temp_item_recovery_type2].' vs resist:'.$this_starforce[$temp_item_recovery_type2].' = no boost');
            }
        ...
        if ($this_robot_stats['robot_speed'] == 0 && $this_item->recovery_options['success_rate'] > 0){
        ...
        elseif ($this_item->recovery_options['recovery_kind'] == 'weapons' && $this_robot_stats['robot_weapons'] >= $this_robot->robot_base_weapons){
            // Hard code the result to failure
            $this_item->item_results['this_result'] = 'failure';
        }
        // Otherwise if ATTACK recovery but attack is already at 9999
        elseif ($this_item->recovery_options['recovery_kind'] == 'attack' && $this_robot_stats['robot_attack'] >= MMRPG_SETTINGS_STATS_MAX){
            // Hard code the result to failure
            $this_item->item_results['this_result'] = 'failure';
        }
        // Otherwise if DEFENSE recovery but defense is already at 9999
        elseif ($this_item->recovery_options['recovery_kind'] == 'defense' && $this_robot_stats['robot_defense'] >= MMRPG_SETTINGS_STATS_MAX){
            // Hard code the result to failure
            $this_item->item_results['this_result'] = 'failure';
        }
        // Otherwise if SPEED recovery but speed is already at 9999
        elseif ($this_item->recovery_options['recovery_kind'] == 'speed' && $this_robot_stats['robot_speed'] >= MMRPG_SETTINGS_STATS_MAX){
            // Hard code the result to failure
            $this_item->item_results['this_result'] = 'failure';
        }
            ...
            if ($trigger_options['apply_stat_modifiers'] != false && !$this_item->recovery_options['recovery_percent']){
                ...
                if ($this_item->recovery_options['recovery_kind'] == 'energy' && ($this_robot->robot_id != $target_robot->robot_id || $trigger_options['referred_recovery'])){
                    ...
                    // Otherwise if both robots have normal stats, calculate the new amount normally
                        ...
                        $this_battle->events_debug(__FILE__, __LINE__, $this_item->item_token.' | normal_recovery | A:'.$target_robot_stats['robot_attack'].' D:'.$this_robot_stats['robot_defense'].' | '.$this_item->item_results['this_amount'].' = round('.$this_item->item_results['this_amount'].' * ('.$target_robot_stats['robot_attack'].' / '.$this_robot_stats['robot_defense'].')) = '.$temp_new_amount.'');
                        // Update the amount with the new calculation
                        $this_item->item_results['this_amount'] = $temp_new_amount;
                    }
            ...
            if ($trigger_options['apply_type_modifiers'] != false){
                ...
                if ($this_item->item_results['flag_affinity']){
                    ...
                    for ($i = 1; $i <= $loop_count; $i++){
                        ...
                        $this_battle->events_debug(__FILE__, __LINE__, $this_item->item_token.' | flag_affinity ('.$i.'/'.$loop_count.') | '.$this_item->item_results['this_amount'].' = round('.$this_item->item_results['this_amount'].' * '.$this_item->recovery_options['affinity_multiplier'].') = '.$temp_new_amount.'');
                        $this_item->item_results['this_amount'] = $temp_new_amount;
                    }
                }
                ...
                if ($this_item->item_results['flag_resistance']){
                    ...
                    for ($i = 1; $i <= $loop_count; $i++){
                        ...
                        $this_battle->events_debug(__FILE__, __LINE__, $this_item->item_token.' | flag_resistance ('.$i.'/'.$loop_count.') | '.$this_item->item_results['this_amount'].' = round('.$this_item->item_results['this_amount'].' * '.$this_item->recovery_options['resistance_multiplier'].') = '.$temp_new_amount.'');
                        $this_item->item_results['this_amount'] = $temp_new_amount;
                    }
                }
                ...
                if ($this_item->item_results['flag_immunity']){
                    ...
                    for ($i = 1; $i <= $loop_count; $i++){
                        ...
                        $this_battle->events_debug(__FILE__, __LINE__, $this_item->item_token.' | flag_immunity ('.$i.'/'.$loop_count.') | '.$this_item->item_results['this_amount'].' = round('.$this_item->item_results['this_amount'].' * '.$this_item->recovery_options['immunity_multiplier'].') = '.$temp_new_amount.'');
                        $this_item->item_results['this_amount'] = $temp_new_amount;
                    }
                }
            ...
            switch ($recovery_stat_name){
                ...
                case 'robot_attack': {
                    ...
                    if ($this_robot_stats['robot_attack'] > MMRPG_SETTINGS_STATS_MAX){
                        ...
                        $this_item->item_results['this_overkill'] = (MMRPG_SETTINGS_STATS_MAX - $this_robot_stats['robot_attack']) * -1;
                ...
                case 'robot_defense': {
                    ...
                    if ($this_robot_stats['robot_defense'] > MMRPG_SETTINGS_STATS_MAX){
                        // Calculate the overkill amount
                        $this_item->item_results['this_overkill'] = (MMRPG_SETTINGS_STATS_MAX - $this_robot_stats['robot_defense']) * -1;
                        // Calculate the actual recovery amount
                        $this_item->item_results['this_amount'] = $this_item->item_results['this_amount'] - $this_item->item_results['this_overkill'];
                ...
                case 'robot_speed': {
                    ...
                    if ($this_robot_stats['robot_speed'] > MMRPG_SETTINGS_STATS_MAX){
                        // Calculate the overkill amount
                        $this_item->item_results['this_overkill'] = (MMRPG_SETTINGS_STATS_MAX - $this_robot_stats['robot_speed']) * -1;
                        // Calculate the actual recovery amount
                        $this_item->item_results['this_amount'] = $this_item->item_results['this_amount'] - $this_item->item_results['this_overkill'];
                ...
                case 'robot_weapons': {
                    ...
                    if ($this_robot_stats['robot_weapons'] > $this_robot->robot_base_weapons){
                        ...
                        $this_item->item_results['this_overkill'] = ($this_robot->robot_base_weapons - $this_robot_stats['robot_weapons']) * -1;
                        // Calculate the actual recovery amount
                        $this_item->item_results['this_amount'] = $this_item->item_results['this_amount'] - $this_item->item_results['this_overkill'];
