<?
/**
 * Mega Man RPG Skill Damage
 * <p>The skill-specific battle damage class for the Mega Man RPG Prototype.</p>
 */
class rpg_skill_damage extends rpg_damage {

    // Define a trigger for inflicting all types of damage on this robot
    public static function trigger_robot_damage($this_robot, $target_robot, $this_skill, $damage_amount, $trigger_disabled = true, $trigger_options = array()){
        global $db;
        ...
        if (!isset($trigger_options['skill_type'])){ $trigger_options['skill_type'] = $this_skill->damage_options['damage_type']; }
        if (!isset($trigger_options['skill_type2'])){ $trigger_options['skill_type2'] = $this_skill->damage_options['damage_type2']; }
        if (!isset($trigger_options['skill_accuracy'])){ $trigger_options['skill_accuracy'] = 100; }
        ...
        $this_skill_backup_frame = $this_skill->skill_frame;
        ...
        rpg_canvas::apply_camera_action_flags($event_options, $this_robot, $this_skill, 'damage');
        ...
        //if ($this_skill->damage_options['damage_percent'] && $options->damage_amount > 100){ $options->damage_amount = 100; }
        $this_skill->damage_options['damage_amount'] = $options->damage_amount;

        // Collect the damage amount argument from the function
        $this_skill->skill_results['this_amount'] = $options->damage_amount;
        ...
        $this_battle->events_debug(__FILE__, __LINE__, $this_skill->skill_token.' : damage_trigger_options : '.$debug);
        ...
        if ($trigger_options['apply_modifiers'] != false){
            ...
            if ($trigger_options['apply_type_modifiers'] != false && ($this_robot->robot_id != $target_robot->robot_id || $trigger_options['referred_damage'])){
                ...
                // If target robot has affinity to the skill (based on type)
                if ($this_robot->has_affinity($this_skill->damage_options['damage_type']) && !$this_robot->has_weakness($this_skill->damage_options['damage_type2'])){
                    //$this_skill->skill_results['counter_affinities'] += 1;
                    //$this_skill->skill_results['flag_affinity'] = true;
                    return $this_robot->trigger_recovery($target_robot, $this_skill, $options->damage_amount);
                } else {
                    $this_skill->skill_results['flag_affinity'] = false;
                }

                // If target robot has affinity to the skill (based on type2)
                if ($this_robot->has_affinity($this_skill->damage_options['damage_type2']) && !$this_robot->has_weakness($this_skill->damage_options['damage_type'])){
                    $this_skill->skill_results['counter_affinities'] += 1;
                    $this_skill->skill_results['flag_affinity'] = true;
                    return $this_robot->trigger_recovery($target_robot, $this_skill, $options->damage_amount);
                }

                // If this robot has weakness to the skill (based on type)
                if ($this_robot->has_weakness($this_skill->damage_options['damage_type']) && !$this_robot->has_affinity($this_skill->damage_options['damage_type2'])){
                    $this_skill->skill_results['counter_weaknesses'] += 1;
                    $this_skill->skill_results['flag_weakness'] = true;
                } else {
                    $this_skill->skill_results['flag_weakness'] = false;
                }

                // If this robot has weakness to the skill (based on type2)
                if ($this_robot->has_weakness($this_skill->damage_options['damage_type2']) && !$this_robot->has_affinity($this_skill->damage_options['damage_type'])){
                    $this_skill->skill_results['counter_weaknesses'] += 1;
                    $this_skill->skill_results['flag_weakness'] = true;
                }

                // If target robot has resistance tp the skill (based on type)
                if ($this_robot->has_resistance($this_skill->damage_options['damage_type'])){
                    $this_skill->skill_results['counter_resistances'] += 1;
                    $this_skill->skill_results['flag_resistance'] = true;
                } else {
                    $this_skill->skill_results['flag_resistance'] = false;
                }

                // If target robot has resistance tp the skill (based on type2)
                if ($this_robot->has_resistance($this_skill->damage_options['damage_type2'])){
                    $this_skill->skill_results['counter_resistances'] += 1;
                    $this_skill->skill_results['flag_resistance'] = true;
                }

                // If target robot has immunity to the skill (based on type)
                if ($this_robot->has_immunity($this_skill->damage_options['damage_type'])){
                    $this_skill->skill_results['counter_immunities'] += 1;
                    $this_skill->skill_results['flag_immunity'] = true;
                } else {
                    $this_skill->skill_results['flag_immunity'] = false;
                }

                // If target robot has immunity to the skill (based on type2)
                if ($this_robot->has_immunity($this_skill->damage_options['damage_type2'])){
                    $this_skill->skill_results['counter_immunities'] += 1;
                    $this_skill->skill_results['flag_immunity'] = true;
                }
            ...
            if ($trigger_options['apply_core_modifiers'] != false){
                ...
                // Collect this skill's type tokens if they exist
                $skill_type_token = !empty($this_skill->damage_options['damage_type']) ? $this_skill->damage_options['damage_type'] : 'none';
                $skill_type_token2 = !empty($this_skill->damage_options['damage_type2']) ? $this_skill->damage_options['damage_type2'] : '';
                ...
                $core_type_token3 = '';
                if (!empty($target_robot->robot_skill) && strstr($target_robot->robot_skill, '-core')){
                    $core_type_token3 = str_replace('-core', '', $target_robot->robot_skill);
                }

                // Define the coreboost flag and default to false
                $this_skill->skill_results['flag_coreboost'] = false;

                // Define an array to hold individual coreboost values
                $skill_coreboost_multipliers = array();

                // Check this skill's FIRST type for multiplier matches
                if (!empty($skill_type_token)){

                    // Apply primary robot core multipliers if they exist
                    if ($skill_type_token == $core_type_token){
                        $this_skill->skill_results['counter_coreboosts']++;
                        $skill_coreboost_multipliers[] = MMRPG_SETTINGS_COREBOOST_MULTIPLIER;
                    }
                    // Apply secondary robot core multipliers if they exist
                    elseif ($skill_type_token == $core_type_token2){
                        $this_skill->skill_results['counter_coreboosts']++;
                        $skill_coreboost_multipliers[] = MMRPG_SETTINGS_COREBOOST_MULTIPLIER;
                    }

                    // Apply held robot core multipliers if they exist
                    if ($skill_type_token == $core_type_token3){
                        $this_skill->skill_results['counter_coreboosts']++;
                        $skill_coreboost_multipliers[] = MMRPG_SETTINGS_SUBCOREBOOST_MULTIPLIER;
                    }

                }

                // Check this skill's SECOND type for multiplier matches
                if (!empty($skill_type_token2)){

                    // Apply primary robot core multipliers if they exist
                    if ($skill_type_token2 == $core_type_token){
                        $this_skill->skill_results['counter_coreboosts']++;
                        $skill_coreboost_multipliers[] = MMRPG_SETTINGS_COREBOOST_MULTIPLIER;
                    }
                    // Apply secondary robot core multipliers if they exist
                    elseif ($skill_type_token2 == $core_type_token2){
                        $this_skill->skill_results['counter_coreboosts']++;
                        $skill_coreboost_multipliers[] = MMRPG_SETTINGS_COREBOOST_MULTIPLIER;
                    }

                    // Apply held robot core multipliers if they exist
                    if ($skill_type_token2 == $core_type_token3){
                        $this_skill->skill_results['counter_coreboosts']++;
                        $skill_coreboost_multipliers[] = MMRPG_SETTINGS_SUBCOREBOOST_MULTIPLIER;
                    }
                ...
                if (in_array('flag_coreboost', $trigger_options['force_flags'])){
                    $this_skill->skill_results['counter_coreboosts'] += 1;
                    $this_skill->skill_results['flag_coreboost'] = true;
                }

                // If any coreboosts were present, update the flag
                if (!empty($this_skill->skill_results['counter_coreboosts'])){
                    $this_skill->skill_results['flag_coreboost'] = true;
                }
            ...
            if ($trigger_options['apply_position_modifiers'] != false){
                ...
                if ($this_robot->robot_position != 'active'){
                    ...
                    $this_battle->events_debug(__FILE__, __LINE__, $this_skill->skill_token.' | position_modifier_damage | '.$options->damage_amount.' = rpg_functions::round_ceil('.$options->damage_amount.' * '.$temp_damage_resistor.') = '.$new_damage_amount.'');
        ...
        if ($trigger_options['apply_field_modifiers'] != false && $this_skill->damage_options['damage_modifiers'] && !empty($this_robot->field->field_multipliers)){

            // Collect the multipliters for easier
            $field_multipliers = $this_robot->field->field_multipliers;

            // Collect the skill types else "none" for multipliers
            $temp_skill_damage_type = !empty($this_skill->damage_options['damage_type']) ? $this_skill->damage_options['damage_type'] : 'none';
            $temp_skill_damage_type2 = !empty($this_skill->damage_options['damage_type2']) ? $this_skill->damage_options['damage_type2'] : '';
            ...
            if (isset($field_multipliers['damage'])){
                ...
                $this_battle->events_debug(__FILE__, __LINE__, $this_skill->skill_token.' | field_multiplier_damage | '.$options->damage_amount.' = rpg_functions::round_ceil('.$options->damage_amount.' * '.$field_multipliers['damage'].') = '.$new_damage_amount.'');
                $options->damage_amount = $new_damage_amount;
            }

            // Loop through all the other type multipliers one by one if this skill has a type
            ...
            foreach ($field_multipliers AS $temp_type => $temp_multiplier){
                ...
                // If this skill's type matches the multiplier, apply it
                if ($temp_skill_damage_type == $temp_type){
                    $new_damage_amount = rpg_functions::round_ceil($options->damage_amount * $temp_multiplier);
                    $this_battle->events_debug(__FILE__, __LINE__, $this_skill->skill_token.' | field_multiplier_'.$temp_type.' | '.$options->damage_amount.' = rpg_functions::round_ceil('.$options->damage_amount.' * '.$temp_multiplier.') = '.$new_damage_amount.'');
                    $options->damage_amount = $new_damage_amount;
                }
                // If this skill's type2 matches the multiplier, apply it
                if ($temp_skill_damage_type2 == $temp_type){
                    $new_damage_amount = rpg_functions::round_ceil($options->damage_amount * $temp_multiplier);
                    $this_battle->events_debug(__FILE__, __LINE__, $this_skill->skill_token.' | field_multiplier_'.$temp_type.' | '.$options->damage_amount.' = rpg_functions::round_ceil('.$options->damage_amount.' * '.$temp_multiplier.') = '.$new_damage_amount.'');
                    $options->damage_amount = $new_damage_amount;
                }
        ...
        // Update the skill results with the the trigger kind and damage details
        $this_skill->skill_results['trigger_kind'] = 'damage';
        $this_skill->skill_results['damage_kind'] = $this_skill->damage_options['damage_kind'];
        $this_skill->skill_results['damage_type'] = $this_skill->damage_options['damage_type'];
        $this_skill->skill_results['damage_type2'] = !empty($this_skill->damage_options['damage_type2']) ? $this_skill->damage_options['damage_type2'] : '';
        ...
        if ($this_skill->damage_options['success_rate'] == 'auto'){
            ...
            if ($this_robot->robot_id == $target_robot->robot_id){
                ...
                $this_skill->damage_options['success_rate'] = $trigger_options['skill_accuracy'];
            }
            // Otherwise, if this robot is in speed break or skill accuracy 100%
            ...
            // Otherwise, if this robot is in speed break or skill accuracy 100%
            elseif ($this_robot->robot_speed <= 0 || $trigger_options['skill_accuracy'] == 100){
                // Hard-code the success rate at 100% accuracy
                    $this_skill->damage_options['success_rate'] = 100;
            }
            // Otherwise, calculate the success rate based on relative speeds
            else {
                // Collect this skill's accuracy stat for modification
                $this_skill_accuracy = $trigger_options['skill_accuracy'];
                // If the target was faster/slower, boost/lower the skill accuracy
                if ($target_robot_stats['robot_speed'] > $this_robot->robot_speed
                    ...
                    //$this_skill_accuracy = ceil($this_skill_accuracy * $this_modifier);
                    $this_skill_accuracy = ceil($this_skill_accuracy * 0.95) + ceil(($this_skill_accuracy * 0.05) * $this_modifier);
                    if ($this_skill_accuracy > 100){ $this_skill_accuracy = 100; }
                    elseif ($this_skill_accuracy < 0){ $this_skill_accuracy = 0; }
                }
                // Update the success rate to the skill accuracy value
        ...
        if ($this_skill->damage_options['failure_rate'] == 'auto'){
            // Set the failure rate to the difference of success vs failure (100% base)
            $this_skill->damage_options['failure_rate'] = 100 - $this_skill->damage_options['success_rate'];
            if ($this_skill->damage_options['failure_rate'] < 0){
                $this_skill->damage_options['failure_rate'] = 0;
            }
        }

        // If this robot is in speed break, increase success rate, reduce failure
        if ($this_robot->robot_speed == 0 && $this_skill->damage_options['success_rate'] > 0){
            $this_skill->damage_options['success_rate'] = ceil($this_skill->damage_options['success_rate'] * 2);
            $this_skill->damage_options['failure_rate'] = ceil($this_skill->damage_options['failure_rate'] / 2);
        }
        // If the target robot is in speed break, decease the success rate, increase failure
        elseif ($target_robot_stats['robot_speed'] == 0 && $this_skill->damage_options['success_rate'] > 0){
            $this_skill->damage_options['success_rate'] = ceil($this_skill->damage_options['success_rate'] / 2);
            $this_skill->damage_options['failure_rate'] = ceil($this_skill->damage_options['failure_rate'] * 2);
        }

        // If success rate is at 100%, auto-set the result to success
        if ($this_skill->damage_options['success_rate'] == 100){
            // Set this skill result as a success
            $this_skill->damage_options['failure_rate'] = 0;
            $this_skill->skill_results['this_result'] = 'success';
        }
        // Else if the success rate is at 0%, auto-set the result to failure
        elseif ($this_skill->damage_options['success_rate'] == 0){
            // Set this skill result as a failure
            $this_skill->damage_options['failure_rate'] = 100;
        ...
        // Otherwise, use a weighted random generation to get the result
            ...
            $this_skill->skill_results['this_result'] = $this_robot->battle->weighted_chance(
                array('success','failure'),
                array($this_skill->damage_options['success_rate'], $this_skill->damage_options['failure_rate'])
                );
        }

        // If this is ENERGY damage and this robot is already disabled
        if ($this_skill->damage_options['damage_kind'] == 'energy' && $this_robot->robot_energy <= 0){
            // Hard code the result to failure
            $this_skill->skill_results['this_result'] = 'failure';
        }
        // If this is WEAPONS recovery and this robot is already at empty ammo
        elseif ($this_skill->damage_options['damage_kind'] == 'weapons' && $this_robot->robot_weapons <= 0){
            // Hard code the result to failure
            $this_skill->skill_results['this_result'] = 'failure';
        }
        // Otherwise if ATTACK damage but attack is already zero
        elseif ($this_skill->damage_options['damage_kind'] == 'attack' && $this_robot->robot_attack <= 0){
            // Hard code the result to failure
            $this_skill->skill_results['this_result'] = 'failure';
        }
        // Otherwise if DEFENSE damage but defense is already zero
        elseif ($this_skill->damage_options['damage_kind'] == 'defense' && $this_robot->robot_defense <= 0){
            // Hard code the result to failure
            $this_skill->skill_results['this_result'] = 'failure';
        }
        // Otherwise if SPEED damage but speed is already zero
        elseif ($this_skill->damage_options['damage_kind'] == 'speed' && $this_robot->robot_speed <= 0){
            // Hard code the result to failure
            $this_skill->skill_results['this_result'] = 'failure';
        }
        ...
        if (!empty($this_skill->skill_results['flag_immunity'])){
            ...
            if (isset($this_skill->damage_options[$this_flag_name])){
                $this_skill->skill_results['this_text'] .= ' '.$this_skill->damage_options[$this_flag_name].'<br /> ';
            }
        }
        ...
        if ($this_skill->skill_results['this_result'] == 'success'){
            ...
            if ($trigger_options['apply_modifiers'] != false){
                ...
                // Update this robot's internal flags based on skill effects
                if (!empty($this_skill->skill_results['flag_weakness'])){
                    ...
                    if ($this_skill->damage_options['damage_kind'] == 'energy' && $this_robot->player->player_side == 'right'){
                        if ($this_battle->flags['allow_experience_points']){ $this_robot->field->field_multipliers['experience'] += 0.1; }
                        $this_skill->damage_options['damage_kickback']['x'] = ceil($this_skill->damage_options['damage_kickback']['x'] * 2);
                    }
                }
                if (!empty($this_skill->skill_results['flag_affinity'])){
                ...
                if (!empty($this_skill->skill_results['flag_critical'])){
            ...
            $this_robot->robot_frame = $this_skill->damage_options['damage_frame'];
            $this_robot->player->player_frame = ($this_robot->robot_id != $target_robot->robot_id || $trigger_options['referred_damage']) ? 'damage' : 'base';
            $this_skill->skill_frame = $this_skill->damage_options['skill_success_frame'];
            $this_skill->skill_frame_span = $this_skill->damage_options['skill_success_frame_span'];
            $this_skill->skill_frame_offset = $this_skill->damage_options['skill_success_frame_offset'];

            // Display the success text, if text has been provided
            if (!empty($this_skill->damage_options['success_text'])){
                $this_skill->skill_results['this_text'] .= $this_skill->damage_options['success_text'];
            }
            ...
            if ($trigger_options['apply_core_modifiers'] != false){
                ...
                // If target robot has core boost for the skill (based on type)
                if ($this_skill->skill_results['flag_coreboost']){
                    foreach ($skill_coreboost_multipliers AS $temp_multiplier){
                        $this_skill->skill_results['this_amount'] = ceil($this_skill->skill_results['this_amount'] * $temp_multiplier);
                        $this_battle->events_debug(__FILE__, __LINE__, $this_skill->skill_token.' | apply_core_modifiers | x '.$temp_multiplier.' = '.$this_skill->skill_results['this_amount'].'');
                    }
                }
            ...
            if ($trigger_options['apply_stat_modifiers'] != false && !$this_skill->damage_options['damage_percent']){

                // Only apply ATTACK/DEFENSE mods if this robot is not targetting itself and it's ENERGY based damage
                if ($this_skill->damage_options['damage_kind'] == 'energy' && ($this_robot->robot_id != $target_robot->robot_id || $trigger_options['referred_damage'])){

                    // Backup the current ammount before stat multipliers
                    $temp_amount_backup = $this_skill->skill_results['this_amount'];
                    ...
                    if ($this_robot->robot_defense <= 0 && $target_robot_stats['robot_attack'] >= 1){
                        ...
                        $this_battle->events_debug(__FILE__, __LINE__, $this_skill->skill_token.' | '.$this_robot->robot_token.'_defense_break | D:'.$this_robot->robot_defense.' | '.$this_skill->skill_results['this_amount'].' = '.$temp_new_amount.'');
                        // Update the amount with the new calculation
                        $this_skill->skill_results['this_amount'] = $temp_new_amount;
                    }
                    ...
                    elseif ($target_robot_stats['robot_attack'] <= 0 && $this_robot->robot_defense >= 1){
                        ...
                        $temp_new_amount = 0;
                        $this_battle->events_debug(__FILE__, __LINE__, $this_skill->skill_token.' | '.$target_robot->robot_token.'_attack_break | A:'.$target_robot_stats['robot_attack'].' | '.$this_skill->skill_results['this_amount'].' = '.$temp_new_amount.'');
                        // Update the amount with the new calculation
                        $this_skill->skill_results['this_amount'] = $temp_new_amount;
                    }
                    ...
                    elseif ($this_robot->robot_defense <= 0 && $target_robot_stats['robot_attack'] <= 0){
                        // Set the new damage amount to NOKO this robot
                        $temp_new_amount = 0;
                        $this_battle->events_debug(__FILE__, __LINE__, $this_skill->skill_token.' | '.$target_robot->robot_token.'_attack_break and '.$this_robot->robot_token.'_defense_break | A:'.$target_robot_stats['robot_attack'].' D:'.$this_robot->robot_defense.' | '.$this_skill->skill_results['this_amount'].' = '.$temp_new_amount.'');
                        // Update the amount with the new calculation
                        $this_skill->skill_results['this_amount'] = $temp_new_amount;
                    }
                    // Otherwise if both robots have normal stats, calculate the new amount normally
                        ...
                        $temp_new_amount = rpg_functions::round_ceil($this_skill->skill_results['this_amount'] * ($target_robot_stats['robot_attack'] / $this_robot->robot_defense));
                        $this_battle->events_debug(__FILE__, __LINE__, $this_skill->skill_token.' | normal_damage | A:'.$target_robot_stats['robot_attack'].' D:'.$this_robot->robot_defense.' | '.$this_skill->skill_results['this_amount'].' = rpg_functions::round_ceil('.$this_skill->skill_results['this_amount'].' * ('.$target_robot_stats['robot_attack'].' / '.$this_robot->robot_defense.')) = '.$temp_new_amount.'');
                        // Update the amount with the new calculation
                        $this_skill->skill_results['this_amount'] = $temp_new_amount;
                    }

                    // If this robot started out above zero but is now absolute zero, round up
                    if ($temp_amount_backup > 0 && $this_skill->skill_results['this_amount'] == 0){ $this_skill->skill_results['this_amount'] = 1; }

                }

                // If this is a critical hit (random chance)
                $critical_rate = $this_skill->damage_options['critical_rate'];
                if ($this_robot->battle->critical_chance($critical_rate)){
                    $this_skill->skill_results['this_amount'] = $this_skill->skill_results['this_amount'] * $this_skill->damage_options['critical_multiplier'];
                    $this_skill->skill_results['flag_critical'] = true;
                    $this_battle->events_debug(__FILE__, __LINE__, $this_skill->skill_token.' | flag_critical | x '.$this_skill->damage_options['critical_multiplier'].' = '.$this_skill->skill_results['this_amount'].'');
                } else {
                    $this_skill->skill_results['flag_critical'] = false;
                }
            ...
            if ($trigger_options['apply_type_modifiers'] != false){
                ...
                // If this robot has a weakness to the skill (based on type)
                if ($this_skill->skill_results['flag_weakness']){
                    $loop_count = $this_skill->skill_results['counter_weaknesses'] / ($this_skill->skill_results['total_strikes'] + 1);
                    for ($i = 1; $i <= $loop_count; $i++){
                        $temp_new_amount = rpg_functions::round_ceil($this_skill->skill_results['this_amount'] * $this_skill->damage_options['weakness_multiplier']);
                        $this_battle->events_debug(__FILE__, __LINE__, $this_skill->skill_token.' | flag_weakness ('.$i.'/'.$loop_count.') | '.$this_skill->skill_results['this_amount'].' = rpg_functions::round_ceil('.$this_skill->skill_results['this_amount'].' * '.$this_skill->damage_options['weakness_multiplier'].') = '.$temp_new_amount.'');
                        $this_skill->skill_results['this_amount'] = $temp_new_amount;
                    }
                }

                // If target robot resists the skill (based on type)
                if ($this_skill->skill_results['flag_resistance']){
                    $loop_count = $this_skill->skill_results['counter_resistances'] / ($this_skill->skill_results['total_strikes'] + 1);
                    for ($i = 1; $i <= $loop_count; $i++){
                        $temp_new_amount = rpg_functions::round_ceil($this_skill->skill_results['this_amount'] * $this_skill->damage_options['resistance_multiplier']);
                        $this_battle->events_debug(__FILE__, __LINE__, $this_skill->skill_token.' | flag_resistance ('.$i.'/'.$loop_count.') | '.$this_skill->skill_results['this_amount'].' = rpg_functions::round_ceil('.$this_skill->skill_results['this_amount'].' * '.$this_skill->damage_options['resistance_multiplier'].') = '.$temp_new_amount.'');
                        $this_skill->skill_results['this_amount'] = $temp_new_amount;
                    }
                }

                // If target robot is immune to the skill (based on type)
                if ($this_skill->skill_results['flag_immunity']){
                    $loop_count = $this_skill->skill_results['counter_immunities'] / ($this_skill->skill_results['total_strikes'] + 1);
                    for ($i = 1; $i <= $loop_count; $i++){
                        $this_skill->skill_results['this_amount'] = rpg_functions::round_ceil($this_skill->skill_results['this_amount'] * $this_skill->damage_options['immunity_multiplier']);
                        $this_battle->events_debug(__FILE__, __LINE__, $this_skill->skill_token.' | flag_immunity ('.$i.'/'.$loop_count.') | '.$this_skill->skill_results['this_amount'].' = rpg_functions::round_ceil('.$this_skill->skill_results['this_amount'].' * '.$this_skill->damage_options['immunity_multiplier'].') = '.$temp_new_amount.'');
                        $this_skill->skill_results['this_amount'] = $temp_new_amount;
                    }
                }
            ...
            if ($trigger_options['apply_modifiers'] != false
                ...
                if ($apply_origin_attachment_modifiers
                    ...
                    foreach ($target_robot_attachments AS $temp_token => $temp_info){
                        $temp_token_debug = str_replace('skill_', 'attachment_', $temp_token);
                        ...
                        if (true){
                            ...
                            if ($apply_origin_attachment_damage_breakers
                                ...
                                $temp_new_amount = ($this_skill->skill_results['this_amount'] * $temp_info['attachment_damage_breaker']);
                                $this_battle->events_debug(__FILE__, __LINE__, 'skill_'.$this_skill->skill_token.' vs. '.$temp_token_debug.' <br /> attachment_damage_breaker | '.$this_skill->skill_results['this_amount'].' = ('.$this_skill->skill_results['this_amount'].' * '.$temp_info['attachment_damage_breaker'].') = '.$temp_new_amount.'');
                                $this_skill->skill_results['this_amount'] = $temp_new_amount;
                            }
                            ...
                            if ($apply_origin_attachment_damage_boosters
                                ...
                                $temp_new_amount = ($this_skill->skill_results['this_amount'] * $temp_info['attachment_damage_booster']);
                                $this_battle->events_debug(__FILE__, __LINE__, 'skill_'.$this_skill->skill_token.' vs. '.$temp_token_debug.' <br /> attachment_damage_booster | '.$this_skill->skill_results['this_amount'].' = ('.$this_skill->skill_results['this_amount'].' * '.$temp_info['attachment_damage_booster'].') = '.$temp_new_amount.'');
                                $this_skill->skill_results['this_amount'] = $temp_new_amount;
                            }
                            ...
                            if ($apply_origin_attachment_damage_breakers
                                ...
                                // Apply the damage breaker multiplier to the current damage amount
                                $temp_new_amount = ($this_skill->skill_results['this_amount'] * $temp_info['attachment_damage_output_breaker']);
                                $this_battle->events_debug(__FILE__, __LINE__, 'skill_'.$this_skill->skill_token.' vs. '.$temp_token_debug.' <br /> attachment_damage_output_breaker | '.$this_skill->skill_results['this_amount'].' = ('.$this_skill->skill_results['this_amount'].' * '.$temp_info['attachment_damage_output_breaker'].') = '.$temp_new_amount.'');
                                $this_skill->skill_results['this_amount'] = $temp_new_amount;
                            }
                            ...
                            if ($apply_origin_attachment_damage_boosters
                                ...
                                // Apply the damage booster multiplier to the current damage amount
                                $temp_new_amount = ($this_skill->skill_results['this_amount'] * $temp_info['attachment_damage_output_booster']);
                                $this_battle->events_debug(__FILE__, __LINE__, 'skill_'.$this_skill->skill_token.' vs. '.$temp_token_debug.' <br /> attachment_damage_output_booster | '.$this_skill->skill_results['this_amount'].' = ('.$this_skill->skill_results['this_amount'].' * '.$temp_info['attachment_damage_output_booster'].') = '.$temp_new_amount.'');
                                $this_skill->skill_results['this_amount'] = $temp_new_amount;
                            }
                        }
                        // Next check to see if any breakers or boosters for either of this skill's types
                        if (!empty($temp_skill_damage_type)){
                            // If this robot's attachment has a damage breaker value set
                            if ($apply_origin_attachment_damage_breakers
                                && isset($temp_info['attachment_damage_breaker_'.$temp_skill_damage_type])){
                                // Apply the damage breaker multiplier to the current damage amount
                                $temp_new_amount = ($this_skill->skill_results['this_amount'] * $temp_info['attachment_damage_breaker_'.$temp_skill_damage_type]);
                                $this_battle->events_debug(__FILE__, __LINE__, 'skill_'.$this_skill->skill_token.' vs. '.$temp_token_debug.' <br /> attachment_damage_breaker_'.$temp_skill_damage_type.' | '.$this_skill->skill_results['this_amount'].' = ('.$this_skill->skill_results['this_amount'].' * '.$temp_info['attachment_damage_breaker_'.$temp_skill_damage_type].') = '.$temp_new_amount.'');
                                $this_skill->skill_results['this_amount'] = $temp_new_amount;
                            }
                            // If this robot's attachment has a damage booster value set
                            if ($apply_origin_attachment_damage_boosters
                                && isset($temp_info['attachment_damage_booster_'.$temp_skill_damage_type])){
                                // Apply the damage booster multiplier to the current damage amount
                                $temp_new_amount = ($this_skill->skill_results['this_amount'] * $temp_info['attachment_damage_booster_'.$temp_skill_damage_type]);
                                $this_battle->events_debug(__FILE__, __LINE__, 'skill_'.$this_skill->skill_token.' vs. '.$temp_token_debug.' <br /> attachment_damage_booster_'.$temp_skill_damage_type.' | '.$this_skill->skill_results['this_amount'].' = ('.$this_skill->skill_results['this_amount'].' * '.$temp_info['attachment_damage_booster_'.$temp_skill_damage_type].') = '.$temp_new_amount.'');
                                $this_skill->skill_results['this_amount'] = $temp_new_amount;
                            }
                            // If this robot's attachment has a damage breaker value set
                            if ($apply_origin_attachment_damage_breakers
                                && isset($temp_info['attachment_damage_output_breaker_'.$temp_skill_damage_type])){
                                // Apply the damage breaker multiplier to the current damage amount
                                $temp_new_amount = ($this_skill->skill_results['this_amount'] * $temp_info['attachment_damage_output_breaker_'.$temp_skill_damage_type]);
                                $this_battle->events_debug(__FILE__, __LINE__, 'skill_'.$this_skill->skill_token.' vs. '.$temp_token_debug.' <br /> attachment_damage_output_breaker_'.$temp_skill_damage_type.' | '.$this_skill->skill_results['this_amount'].' = ('.$this_skill->skill_results['this_amount'].' * '.$temp_info['attachment_damage_output_breaker_'.$temp_skill_damage_type].') = '.$temp_new_amount.'');
                                $this_skill->skill_results['this_amount'] = $temp_new_amount;
                            }
                            // If this robot's attachment has a damage booster value set
                            if ($apply_origin_attachment_damage_boosters
                                && isset($temp_info['attachment_damage_output_booster_'.$temp_skill_damage_type])){
                                // Apply the damage booster multiplier to the current damage amount
                                $temp_new_amount = ($this_skill->skill_results['this_amount'] * $temp_info['attachment_damage_output_booster_'.$temp_skill_damage_type]);
                                $this_battle->events_debug(__FILE__, __LINE__, 'skill_'.$this_skill->skill_token.' vs. '.$temp_token_debug.' <br /> attachment_damage_output_booster_'.$temp_skill_damage_type.' | '.$this_skill->skill_results['this_amount'].' = ('.$this_skill->skill_results['this_amount'].' * '.$temp_info['attachment_damage_output_booster_'.$temp_skill_damage_type].') = '.$temp_new_amount.'');
                                $this_skill->skill_results['this_amount'] = $temp_new_amount;
                            }
                        }
                        // Next check to see if any breakers or boosters for either of this skill's types
                        if (!empty($temp_skill_damage_type2)){
                            // If this robot's attachment has a damage breaker value set
                            if ($apply_origin_attachment_damage_breakers
                                && isset($temp_info['attachment_damage_breaker_'.$temp_skill_damage_type2])){
                                // Apply the damage breaker multiplier to the current damage amount
                                $temp_new_amount = ($this_skill->skill_results['this_amount'] * $temp_info['attachment_damage_breaker_'.$temp_skill_damage_type2]);
                                $this_battle->events_debug(__FILE__, __LINE__, 'skill_'.$this_skill->skill_token.' vs. '.$temp_token_debug.' <br /> attachment_damage_breaker_'.$temp_skill_damage_type2.' | '.$this_skill->skill_results['this_amount'].' = ('.$this_skill->skill_results['this_amount'].' * '.$temp_info['attachment_damage_breaker_'.$temp_skill_damage_type2].') = '.$temp_new_amount.'');
                                $this_skill->skill_results['this_amount'] = $temp_new_amount;
                            }
                            // If this robot's attachment has a damage booster value set
                            if ($apply_origin_attachment_damage_boosters
                                && isset($temp_info['attachment_damage_booster_'.$temp_skill_damage_type2])){
                                // Apply the damage booster multiplier to the current damage amount
                                $temp_new_amount = ($this_skill->skill_results['this_amount'] * $temp_info['attachment_damage_booster_'.$temp_skill_damage_type2]);
                                $this_battle->events_debug(__FILE__, __LINE__, 'skill_'.$this_skill->skill_token.' vs. '.$temp_token_debug.' <br /> attachment_damage_booster_'.$temp_skill_damage_type2.' | '.$this_skill->skill_results['this_amount'].' = ('.$this_skill->skill_results['this_amount'].' * '.$temp_info['attachment_damage_booster_'.$temp_skill_damage_type2].') = '.$temp_new_amount.'');
                                $this_skill->skill_results['this_amount'] = $temp_new_amount;
                            }
                            // If this robot's attachment has a damage breaker value set
                            if ($apply_origin_attachment_damage_breakers
                                && isset($temp_info['attachment_damage_output_breaker_'.$temp_skill_damage_type2])){
                                // Apply the damage breaker multiplier to the current damage amount
                                $temp_new_amount = ($this_skill->skill_results['this_amount'] * $temp_info['attachment_damage_output_breaker_'.$temp_skill_damage_type2]);
                                $this_battle->events_debug(__FILE__, __LINE__, 'skill_'.$this_skill->skill_token.' vs. '.$temp_token_debug.' <br /> attachment_damage_output_breaker_'.$temp_skill_damage_type2.' | '.$this_skill->skill_results['this_amount'].' = ('.$this_skill->skill_results['this_amount'].' * '.$temp_info['attachment_damage_output_breaker_'.$temp_skill_damage_type2].') = '.$temp_new_amount.'');
                                $this_skill->skill_results['this_amount'] = $temp_new_amount;
                            }
                            // If this robot's attachment has a damage booster value set
                            if ($apply_origin_attachment_damage_boosters
                                && isset($temp_info['attachment_damage_output_booster_'.$temp_skill_damage_type2])){
                                // Apply the damage booster multiplier to the current damage amount
                                $temp_new_amount = ($this_skill->skill_results['this_amount'] * $temp_info['attachment_damage_output_booster_'.$temp_skill_damage_type2]);
                                $this_battle->events_debug(__FILE__, __LINE__, 'skill_'.$this_skill->skill_token.' vs. '.$temp_token_debug.' <br /> attachment_damage_output_booster_'.$temp_skill_damage_type2.' | '.$this_skill->skill_results['this_amount'].' = ('.$this_skill->skill_results['this_amount'].' * '.$temp_info['attachment_damage_output_booster_'.$temp_skill_damage_type2].') = '.$temp_new_amount.'');
                                $this_skill->skill_results['this_amount'] = $temp_new_amount;
                            }
                        }
                    ...
                    foreach ($this_robot_attachments AS $temp_token => $temp_info){
                        ...
                        if (true){
                            ...
                            if ($apply_target_attachment_damage_breakers
                                ...
                                // Apply the damage breaker multiplier to the current damage amount
                                $temp_new_amount = ($this_skill->skill_results['this_amount'] * $temp_info['attachment_damage_input_breaker']);
                                $this_battle->events_debug(__FILE__, __LINE__, 'skill_'.$this_skill->skill_token.' vs. '.$temp_token_debug.' <br /> attachment_damage_input_breaker | '.$this_skill->skill_results['this_amount'].' = ('.$this_skill->skill_results['this_amount'].' * '.$temp_info['attachment_damage_input_breaker'].') = '.$temp_new_amount.'');
                                $this_skill->skill_results['this_amount'] = $temp_new_amount;
                            }
                            ...
                            if ($apply_target_attachment_damage_boosters
                                ...
                                // Apply the damage booster multiplier to the current damage amount
                                $temp_new_amount = ($this_skill->skill_results['this_amount'] * $temp_info['attachment_damage_input_booster']);
                                $this_battle->events_debug(__FILE__, __LINE__, 'skill_'.$this_skill->skill_token.' vs. '.$temp_token_debug.' <br /> attachment_damage_input_booster | '.$this_skill->skill_results['this_amount'].' = ('.$this_skill->skill_results['this_amount'].' * '.$temp_info['attachment_damage_input_booster'].') = '.$temp_new_amount.'');
                                $this_skill->skill_results['this_amount'] = $temp_new_amount;
                            }
                        }
                        // Next check to see if any breakers or boosters for either of this skill's types
                        if (!empty($temp_skill_damage_type)){
                            ...
                            // If this robot's attachment has a damage breaker value set
                            if ($apply_target_attachment_damage_breakers
                                && isset($temp_info['attachment_damage_input_breaker_'.$temp_skill_damage_type])){
                                // Apply the damage breaker multiplier to the current damage amount
                                $temp_new_amount = ($this_skill->skill_results['this_amount'] * $temp_info['attachment_damage_input_breaker_'.$temp_skill_damage_type]);
                                $this_battle->events_debug(__FILE__, __LINE__, 'skill_'.$this_skill->skill_token.' vs. '.$temp_token_debug.' <br /> attachment_damage_input_breaker_'.$temp_skill_damage_type.' | '.$this_skill->skill_results['this_amount'].' = ('.$this_skill->skill_results['this_amount'].' * '.$temp_info['attachment_damage_input_breaker_'.$temp_skill_damage_type].') = '.$temp_new_amount.'');
                                $this_skill->skill_results['this_amount'] = $temp_new_amount;
                            }
                            // If this robot's attachment has a damage booster value set
                            if ($apply_target_attachment_damage_boosters
                                && isset($temp_info['attachment_damage_input_booster_'.$temp_skill_damage_type])){
                                // Apply the damage booster multiplier to the current damage amount
                                $temp_new_amount = ($this_skill->skill_results['this_amount'] * $temp_info['attachment_damage_input_booster_'.$temp_skill_damage_type]);
                                $this_battle->events_debug(__FILE__, __LINE__, 'skill_'.$this_skill->skill_token.' vs. '.$temp_token_debug.' <br /> attachment_damage_input_booster_'.$temp_skill_damage_type.' | '.$this_skill->skill_results['this_amount'].' = ('.$this_skill->skill_results['this_amount'].' * '.$temp_info['attachment_damage_input_booster_'.$temp_skill_damage_type].') = '.$temp_new_amount.'');
                                $this_skill->skill_results['this_amount'] = $temp_new_amount;
                            }
                        }
                        // Next check to see if any breakers or boosters for either of this skill's types
                        if (!empty($temp_skill_damage_type2)){
                            ...
                            // If this robot's attachment has a damage breaker value set
                            if ($apply_target_attachment_damage_breakers
                                && isset($temp_info['attachment_damage_input_breaker_'.$temp_skill_damage_type2])){
                                // Apply the damage breaker multiplier to the current damage amount
                                $temp_new_amount = ($this_skill->skill_results['this_amount'] * $temp_info['attachment_damage_input_breaker_'.$temp_skill_damage_type2]);
                                $this_battle->events_debug(__FILE__, __LINE__, 'skill_'.$this_skill->skill_token.' vs. '.$temp_token_debug.' <br /> attachment_damage_input_breaker_'.$temp_skill_damage_type2.' | '.$this_skill->skill_results['this_amount'].' = ('.$this_skill->skill_results['this_amount'].' * '.$temp_info['attachment_damage_input_breaker_'.$temp_skill_damage_type2].') = '.$temp_new_amount.'');
                                $this_skill->skill_results['this_amount'] = $temp_new_amount;
                            }
                            // If this robot's attachment has a damage booster value set
                            if ($apply_target_attachment_damage_boosters
                                && isset($temp_info['attachment_damage_input_booster_'.$temp_skill_damage_type2])){
                                // Apply the damage booster multiplier to the current damage amount
                                $temp_new_amount = ($this_skill->skill_results['this_amount'] * $temp_info['attachment_damage_input_booster_'.$temp_skill_damage_type2]);
                                $this_battle->events_debug(__FILE__, __LINE__, 'skill_'.$this_skill->skill_token.' vs. '.$temp_token_debug.' <br /> attachment_damage_input_booster_'.$temp_skill_damage_type2.' | '.$this_skill->skill_results['this_amount'].' = ('.$this_skill->skill_results['this_amount'].' * '.$temp_info['attachment_damage_input_booster_'.$temp_skill_damage_type2].') = '.$temp_new_amount.'');
                                $this_skill->skill_results['this_amount'] = $temp_new_amount;
                            }
                        }
                ...
                $temp_new_amount = rpg_functions::round_ceil($this_skill->skill_results['this_amount']);
                $this_battle->events_debug(__FILE__, __LINE__, 'skill_'.$this_skill->skill_token.' modifiers applied <br /> round up results | '.$this_skill->skill_results['this_amount'].' = rpg_functions::round_ceil('.$this_skill->skill_results['this_amount'].') = '.$temp_new_amount.'');
                $this_skill->skill_results['this_amount'] = $temp_new_amount;
            ...
            if ($this_skill->skill_results['flag_immunity']){ $this_flag_string[] = 'immunity'; }
            elseif ($trigger_options['apply_type_modifiers'] != false){
                if (!empty($this_skill->skill_results['flag_weakness'])){ $this_flag_string[] = 'weakness'; }
                if (!empty($this_skill->skill_results['flag_affinity'])){ $this_flag_string[] = 'affinity'; }
                if (!empty($this_skill->skill_results['flag_resistance'])){ $this_flag_string[] = 'resistance'; }
                if ($trigger_options['apply_modifiers'] != false && !$this_skill->damage_options['damage_percent']){
                if (!empty($this_skill->skill_results['flag_critical'])){ $this_flag_string[] = 'critical'; }
                }
            }
            ...
            if (isset($this_skill->damage_options[$this_flag_name])){
                ...
                $this_skill->skill_results['this_text'] .= ' '.$this_skill->damage_options[$this_flag_name];
            }

            // Display a break before the damage amount if other text was generated
            if (!empty($this_skill->skill_results['this_text'])){
                $this_skill->skill_results['this_text'] .= '<br />';
            }

            // Ensure the damage amount is always at least one, unless absolute zero
            if ($this_skill->skill_results['this_amount'] < 1 && $this_skill->skill_results['this_amount'] > 0){ $this_skill->skill_results['this_amount'] = 1; }

            // Reference the requested damage kind with a shorter variable
            $this_skill->damage_options['damage_kind'] = strtolower($this_skill->damage_options['damage_kind']);
            $damage_stat_name = 'robot_'.$this_skill->damage_options['damage_kind'];
            ...
            switch ($damage_stat_name){
                ...
                case 'robot_attack': {
                    ...
                    $this_robot->robot_attack = $this_robot->robot_attack - $this_skill->skill_results['this_amount'];
                    ...
                    if ($this_robot->robot_attack < MMRPG_SETTINGS_STATS_MIN){
                        ...
                        $this_skill->skill_results['this_overkill'] = $this_robot->robot_attack * -1;
                        // Calculate the actual damage amount
                        $this_skill->skill_results['this_amount'] = $this_skill->skill_results['this_amount'] + $this_robot->robot_attack;
                ...
                case 'robot_defense': {
                    ...
                    $this_robot->robot_defense = $this_robot->robot_defense - $this_skill->skill_results['this_amount'];
                    // If the damage put the robot's defense below zero
                    if ($this_robot->robot_defense < MMRPG_SETTINGS_STATS_MIN){
                        // Calculate the overkill amount
                        $this_skill->skill_results['this_overkill'] = $this_robot->robot_defense * -1;
                        // Calculate the actual damage amount
                        $this_skill->skill_results['this_amount'] = $this_skill->skill_results['this_amount'] + $this_robot->robot_defense;
                ...
                case 'robot_speed': {
                    // Inflict attack damage on the target's internal stat
                    $this_robot->robot_speed = $this_robot->robot_speed - $this_skill->skill_results['this_amount'];
                    // If the damage put the robot's speed below zero
                    if ($this_robot->robot_speed < MMRPG_SETTINGS_STATS_MIN){
                        // Calculate the overkill amount
                        $this_skill->skill_results['this_overkill'] = $this_robot->robot_speed * -1;
                        // Calculate the actual damage amount
                        $this_skill->skill_results['this_amount'] = $this_skill->skill_results['this_amount'] + $this_robot->robot_speed;
                ...
                case 'robot_weapons': {
                    ...
                    $this_robot->robot_weapons = $this_robot->robot_weapons - $this_skill->skill_results['this_amount'];
                    // If the damage put the robot's weapons below zero
                    if ($this_robot->robot_weapons < MMRPG_SETTINGS_STATS_MIN){
                        // Calculate the overkill amount
                        $this_skill->skill_results['this_overkill'] = $this_robot->robot_weapons * -1;
                        // Calculate the actual damage amount
                        $this_skill->skill_results['this_amount'] = $this_skill->skill_results['this_amount'] + $this_robot->robot_weapons;
                ...
                case 'robot_energy': default: {
                    ...
                    $this_robot->robot_energy = $this_robot->robot_energy - $this_skill->skill_results['this_amount'];
                    // If the damage put the robot into overkill, recalculate the damage
                    if ($this_robot->robot_energy < MMRPG_SETTINGS_STATS_MIN){
                        // Calculate the overkill amount
                        $this_skill->skill_results['this_overkill'] = $this_robot->robot_energy * -1;
                        // Calculate the actual damage amount
                        $this_skill->skill_results['this_amount'] = $this_skill->skill_results['this_amount'] + $this_robot->robot_energy;
            ...
            $this_skill->skill_results['print_strikes'] = '<span class="damage_strikes">'.(!empty($this_skill->skill_results['total_strikes']) ? $this_skill->skill_results['total_strikes'] : 0).'</span>';
            $this_skill->skill_results['print_misses'] = '<span class="damage_misses">'.(!empty($this_skill->skill_results['total_misses']) ? $this_skill->skill_results['total_misses'] : 0).'</span>';
            $this_skill->skill_results['print_result'] = '<span class="damage_result">'.(!empty($this_skill->skill_results['total_result']) ? $this_skill->skill_results['total_result'] : 0).'</span>';
            $this_skill->skill_results['print_amount'] = '<span class="damage_amount">'.(!empty($this_skill->skill_results['this_amount']) ? $this_skill->skill_results['this_amount'] : 0).'</span>';
            $this_skill->skill_results['print_overkill'] = '<span class="damage_overkill">'.(!empty($this_skill->skill_results['this_overkill']) ? $this_skill->skill_results['this_overkill'] : 0).'</span>';

            // Add the final damage text showing the amount based on damage type
            if ($this_skill->damage_options['damage_kind'] == 'energy'){
                $this_skill->skill_results['this_text'] .= "{$this_robot->print_name()} takes {$this_skill->skill_results['print_amount']} life energy damage";
                $this_skill->skill_results['this_text'] .= ($this_skill->skill_results['this_overkill'] > 0 && $this_robot->player->player_side == 'right' ? " and {$this_skill->skill_results['print_overkill']} overkill" : '');
                $this_skill->skill_results['this_text'] .= '!<br />';
            }
            // Otherwise add the final damage text showing the amount based on weapon energy damage
            elseif ($this_skill->damage_options['damage_kind'] == 'weapons'){
                $this_skill->skill_results['this_text'] .= "{$this_robot->print_name()} takes {$this_skill->skill_results['print_amount']} weapon energy damage";
                $this_skill->skill_results['this_text'] .= '!<br />';
            }
            // Otherwise, if this is one of the robot's other internal stats
            elseif ($this_skill->damage_options['damage_kind'] == 'attack'
                || $this_skill->damage_options['damage_kind'] == 'defense'
                || $this_skill->damage_options['damage_kind'] == 'speed'){
                // Print the result based on if the stat will go any lower
                if ($this_skill->skill_results['this_amount'] > 0){
                    $this_skill->skill_results['this_text'] .= "{$this_robot->print_name()}&#39;s {$this_skill->damage_options['damage_kind']} fell by {$this_skill->skill_results['print_amount']}";
                    $this_skill->skill_results['this_text'] .= '!<br />';
                }
                // Otherwise if the stat wouldn't go any lower
                    ...
                    // Update this robot's frame based on damage type
                    $this_skill->skill_frame = $this_skill->damage_options['skill_failure_frame'];
                    $this_skill->skill_frame_span = $this_skill->damage_options['skill_failure_frame_span'];
                    $this_skill->skill_frame_offset = $this_skill->damage_options['skill_failure_frame_offset'];

                    // Display the failure text, if text has been provided
                    if (!empty($this_skill->damage_options['failure_text'])){
                        $this_skill->skill_results['this_text'] .= $this_skill->damage_options['failure_text'].' ';
                    }
                }
            }
        ...
        // Otherwise, if the attack was a failure
            ...
            $this_skill->skill_results['this_amount'] = 0;
            $this_skill->skill_results['this_overkill'] = 0;

            // Display the failure text, if text has been provided
            if (!$this_skill->skill_results['flag_immunity'] && !empty($this_skill->damage_options['failure_text'])){
                $this_skill->skill_results['this_text'] .= $this_skill->damage_options['failure_text'].' ';
            }
        ...
        if ($this_ability->ability_results['this_amount'] > 0){
            ...
            $this_robot->history['triggered_damage'][] = $this_skill->skill_results['this_amount'];
            $this_robot->history['triggered_damage_by'][] = $this_skill->skill_token;

            // Update the robot's history with the triggered damage types
            if (!empty($this_skill->skill_results['damage_type'])){
                $temp_types = array();
                $temp_types[] = $this_skill->skill_results['damage_type'];
                if (!empty($this_skill->skill_results['damage_type2'])){ $temp_types[] = $this_skill->skill_results['damage_type2']; }
        ...
        if (!empty($this_skill->skill_results['this_overkill'])){

            // Collect the overkill amount to boost
            $overkill_value = $this_skill->skill_results['this_overkill'];

            $this_battle->events_debug(__FILE__, __LINE__, $this_robot->robot_token.' | skill overkill | value : '.$overkill_value);
        ...
        $this_skill->skill_results['total_amount'] += !empty($this_skill->skill_results['this_amount']) ? $this_skill->skill_results['this_amount'] : 0;
        $this_skill->skill_results['total_overkill'] += !empty($this_skill->skill_results['this_overkill']) ? $this_skill->skill_results['this_overkill'] : 0;
        if ($this_skill->skill_results['this_result'] == 'success'){ $this_skill->skill_results['total_strikes']++; }
        else { $this_skill->skill_results['total_misses']++; }
        $this_skill->skill_results['total_actions'] = $this_skill->skill_results['total_strikes'] + $this_skill->skill_results['total_misses'];
        if ($this_skill->skill_results['total_result'] != 'success'){ $this_skill->skill_results['total_result'] = $this_skill->skill_results['this_result']; }
        ...
        $this_skill->skill_results['energy_ohko'] = false;
        if ($this_robot->robot_energy <= 0 && $this_robot_energy_start_max){
            $this_battle->events_debug(__FILE__, __LINE__, $this_skill->skill_token.' | damage_result_OHKO! | Start:'.$this_robot_energy_start.' '.($this_robot_energy_start_max ? '(MAX!)' : '-').' | Finish:'.$this_robot->robot_energy);
            // Ensure the attacking player was a human
            if ($this_robot->player->player_side == 'right'){
                $this_skill->skill_results['energy_ohko'] = true;
            }
        }
        ...
        if ($this_skill->skill_results['this_amount'] > 0){
            ...
            } elseif (!empty($this_skill->skill_results['flag_resistance'])){
                $damage_sounds[] = array('name' => 'damage-reduced', 'volume' => 0.9);
            } elseif ($this_skill->skill_results['this_amount'] === 1){
        ...
        if ($this_robot->robot_id == $target_robot->robot_id){
            ...
            $event_options['this_skill_target'] = $this_robot->robot_id.'_'.$this_robot->robot_token;
            ...
            $this_robot->battle->events_create($target_robot, $this_robot, $this_skill->damage_options['damage_header'], $this_skill->skill_results['this_text'], $event_options);
        } else {
            $event_options['console_show_target'] = false;
            $event_options['this_skill_target'] = $this_robot->robot_id.'_'.$this_robot->robot_token;
            ...
            $this_robot->battle->events_create($this_robot, $target_robot, $this_skill->damage_options['damage_header'], $this_skill->skill_results['this_text'], $event_options);
        }
        ...
        $this_skill->skill_frame = $this_skill_backup_frame;
        ...
        if ($this_robot->robot_status == 'disabled' && $trigger_disabled){
            ...
            if ($this_skill->skill_results['energy_ohko']){ $disabled_trigger_options['skill_multiplier'] = 2.0; }
            ...
            if ($this_skill->skill_results['this_result'] == 'success'){
                ...
                if (!empty($this_robot_attachments)){
                    ...
                    foreach ($this_robot_attachments AS $attachment_token => $attachment_info){
                        ...
                        // Ensure this skill has a type before checking weaknesses, resistances, etc.
                        if (!empty($trigger_options['skill_type'])
                                || (isset($attachment_info['attachment_weaknesses']) && in_array('*', $attachment_info['attachment_weaknesses']))){

                            // If this attachment has weaknesses defined and this skill is a match
                            if (!empty($attachment_info['attachment_weaknesses'])
                                && (in_array('*', $attachment_info['attachment_weaknesses'])
                                    || in_array($trigger_options['skill_type'], $attachment_info['attachment_weaknesses'])
                                    || in_array($trigger_options['skill_type2'], $attachment_info['attachment_weaknesses']))
                                ...
                                if ($attachment_destroy_info !== false){
                                    ...
                                    if ($temp_trigger_type == 'damage'){
                                        ...
                                        $this_robot->trigger_target($target_robot, $temp_attachment, array('canvas_show_this_skill' => false, 'prevent_default_text' => true));
                                    }
                                }
                                // If this robot was disabled, process experience for the target
