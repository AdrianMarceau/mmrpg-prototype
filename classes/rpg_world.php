...
class rpg_world {
    ...
    */
    ...
    public static function get_robots_overview_markup($this_prototype_data, $current_player_robots){
        ...
        $return_markup .= '<div class="limit-hearts" data-count="'.$limit_hearts.'">';
    ...
    public static function unlock_player_character($player_token){
        ...
        if (empty($temp_game_flags['events'][$temp_event_flag])){
            ...
            if ($auto_unlock_campaign){
                ...
                $temp_console_markup .= '<p style="margin: 5px auto 10px; text-align: center;">Play through the game as <strong>'.$player_name.'</strong>'.($first_robot !== 'robot' ? ' and <strong>'.$first_robot_name.'</strong>' : '').' to experience events from '.($first_robot === 'robot' ? $player_pronoun : 'their').' perspective! Unlock new robots, items, and abilities using '.($first_robot === 'robot' ? $player_pronoun : 'their').' unique skills while you search for answers. '.$player_unlock_subtext.'</p>';
    ...
    public static function get_player_robot_overview($player_token, $robot_token, $robot_id){
        ...
        $robot_index_info = $mmrpg_index_robots[$robot_token];
        //error_log('$player_index_info = '.print_r($player_index_info, true));
        ...
        if ($has_persona_applied && !empty($robot_settings['robot_persona_image'])){ $robot_image = $robot_settings['robot_persona_image']; }
        ...
        $robot_overview = array(
            ...
            'attackBonus' => (!empty($robot_rewards['robot_attack']) ? $robot_rewards['robot_attack'] : 0),
            'attackMods' => (isset($robot_session['attack']) ? $robot_session['attack'] : 0),
            'defense' => $robot_stats['defense']['current'],
            'defenseBonus' => (!empty($robot_rewards['robot_defense']) ? $robot_rewards['robot_defense'] : 0),
            'defenseMods' => (isset($robot_session['defense']) ? $robot_session['defense'] : 0),
            'speed' => $robot_stats['speed']['current'],
            'speedBonus' => (!empty($robot_rewards['robot_speed']) ? $robot_rewards['robot_speed'] : 0),
