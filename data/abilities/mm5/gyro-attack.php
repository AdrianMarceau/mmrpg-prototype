...
$ability = array(
    ...
    'ability_function' => function($objects){
        ...
        $this_ability->target_options_update(array(
            'frame' => 'throw',
            'success' => array(0, 180, 0, 10, $this_robot->print_robot_name().' throws the '.$this_ability->print_ability_name().'!')
            ));
        ...
        $this_ability->damage_options_update(array(
            ...
            'success' => array(1, -140, 0, 10, 'The '.$this_ability->print_ability_name().'\s blades through the target!'),
            'failure' => array(2, -160, 0, -10, 'The '.$this_ability->print_ability_name().' missed&hellip;')
            ));
        $this_ability->recovery_options_update(array(
            'kind' => 'energy',
            'frame' => 'taunt',
            'kickback' => array(5, 0, 0),
            'success' => array(1, -140, 0, 10, 'The '.$this_ability->print_ability_name().'\'s blades honed the target!'),
            'failure' => array(2, -160, 0, -10, 'The '.$this_ability->print_ability_name().' missed&hellip;')
            ));
        ...
        if (!empty($target_player->values['robots_active'])){
            ...
            if ($last_target_info['robot_id'] != $target_robot->robot_id){ $last_target_robot = new mmrpg_robot($this_battle, $target_player, $last_target_info); }
