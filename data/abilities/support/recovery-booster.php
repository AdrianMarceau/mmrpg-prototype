...
// RECOVERY BOOSTER
$ability = array(
  'ability_name' => 'Recovery Booster',
  'ability_token' => 'recovery-booster',
  'ability_group' => 'MMRPG/Support/Recovery',
  'ability_description' => 'The user alters the conditions of the field to boost the recovery effects of abilities by 30%! This ability appears to be unaffected by existing field multipliers.',
  'ability_energy' => 8,
  'ability_speed' => -2,
  'ability_recovery' => 30,
  ...
  'ability_function' => function($objects){
    ...
    if (isset($this_field->field_multipliers['recovery']) && $this_field->field_multipliers['recovery'] >= MMRPG_SETTINGS_MULTIPLIER_MAX){
      ...
      $this_ability->target_options_update(array(
        ...
        'success' => array(9, 0, 0, -10,
          ...
          'But the field\'s recovery wont go any higher&hellip;'
          )
        ));
    ...
    // Create or increase the recovery booster for this field
    $temp_change_percent = round($this_ability->ability_recovery / 100, 1);
    $temp_new_recovery = $this_field->get_multiplier('recovery') + $temp_change_percent;
    if ($temp_new_recovery > MMRPG_SETTINGS_MULTIPLIER_MAX){
      $temp_change_percent = MMRPG_SETTINGS_MULTIPLIER_MAX - $temp_new_recovery;
      $temp_new_recovery = MMRPG_SETTINGS_MULTIPLIER_MAX;
    }
    $this_field->set_multiplier('recovery', $temp_new_recovery);
    ...
    $this_battle->events_create($this_robot, false, $this_field->field_name.' Multipliers',
    	$random_sayings[array_rand($random_sayings)].' <span class="ability_name ability_type ability_type_recovery">Recovery Effects</span> were boosted by '.ceil($temp_change_percent * 100).'%!<br />'.
