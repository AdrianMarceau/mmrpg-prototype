  ...
  if (!empty($target_player->player_robots)){
    ...
    foreach ($target_player_robots AS $robot_key => $scan_robotinfo){
      ...
      if (!empty($switch_robotinfo['robot_token'])){
        ...
        ?><a class="button <?= !$allow_button ? 'button_disabled' : '' ?> action_scan scan_<?= $temp_robot->robot_token ?> status_<?= $temp_robot->robot_status ?> robot_type robot_type_<?= !empty($temp_robot->robot_core) ? $temp_robot->robot_core : 'none' ?> block_<?= $robot_key + 1 ?>" type="button" data-tooltip="<?=$temp_robot_title_tooltip?>" <?=$order_button_markup?> <?if($allow_button):?>data-action="scan_<?= $temp_robot->robot_id.'_'.$temp_robot->robot_token ?>"<?endif;?> data-preload="<?= $temp_robot_sprite['preload'] ?>"><label><?= $temp_robot_sprite['markup'] ?><?= $temp_robot_label ?></label></a><?
      }
    }
