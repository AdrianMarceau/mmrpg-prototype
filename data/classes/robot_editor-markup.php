        ...
        <tbody>
          <tr>
            ...
            <td  class="right">
              ...
              <? if($robot_info['robot_level'] >= 100): ?>
                <a data-tooltip-align="center" data-tooltip="<?= htmlentities(('Congratulations! '.$robot_info['robot_name'].' has reached Level 100!<br /> <span style="font-size: 90%;">Stat bonuses will now be awarded immediately when this robot lands the finishing blow on a target! Try to max out your stats at '.MMRPG_SETTINGS_STATS_MAX.' each!</span>'), ENT_QUOTES, 'UTF-8') ?>" class="robot_stat robot_type_electric"><span>&#9733;</span> <?= $robot_info['robot_level'] ?></a>
              <? elseif(false && $robot_info['robot_level'] > 1): ?>
                <a title="Reboot Robot?" class="robot_stat robot_level_reset robot_type_<?= !empty($robot_rewards['flags']['reached_max_level']) ? 'electric' : 'cutter' ?>" data-player="<?=$player_info['player_token']?>" data-robot="<?=$robot_info['robot_token']?>" data-level="<?= $robot_info['robot_level'] ?>"><?= !empty($robot_rewards['flags']['reached_max_level']) ? '<span>&#9733;</span>' : '<span>&#8657;</span>' ?> <?= $robot_info['robot_level'] ?></a>
              <? else: ?>
          ...
          <tr>
            ...
            <td  class="right">
              ...
              <span class="robot_stat"><?= $robot_info['robot_level'] < 100 ? $robot_info['robot_experience'] : '&#8734;' ?> / 1000</span>
            </td>
          </tr>
