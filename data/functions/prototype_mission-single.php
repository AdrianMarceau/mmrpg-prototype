...
function mmrpg_prototype_mission_single($this_prototype_data, $this_robot_token, $this_field_token, $this_start_level = 1, $this_unlock_robots = true, $this_unlock_abilities = true, $this_addon_abilities = 0){
  ...
  elseif ($temp_prototype_complete && $temp_battle_complete && $temp_master_unlocked){
  ...
  if ($temp_starforce_battle){
    ...
    foreach ($temp_factors_list AS $this_list){
      ...
      foreach ($this_list AS $this_factor){
        ...
        $bonus_robot_info = $this_robot_index[$this_factor['robot']]; //mmrpg_robot::parse_index_info($this_robot_index[$this_factor['robot']]);
