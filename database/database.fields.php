...
$this_database->query("SET @field_row_number = 0;");
$mmrpg_database_fields = $this_database->get_array_list("SELECT {$field_fields} FROM mmrpg_index_fields WHERE field_flag_published = 1 {$temp_condition} ORDER BY field_order ASC;", 'field_token');
...
$mmrpg_database_fields_numbers = $this_database->get_array_list("SELECT field_token, (@field_row_number:=@field_row_number + 1) AS field_key FROM mmrpg_index_fields WHERE field_flag_published = 1 {$temp_condition_unfiltered} ORDER BY field_flag_hidden ASC, field_order ASC;", 'field_token');
