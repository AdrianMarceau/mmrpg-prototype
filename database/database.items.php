...
$this_database->query("SET @item_row_number = 0;");
$mmrpg_database_items = $this_database->get_array_list("SELECT {$item_fields} FROM mmrpg_index_items WHERE item_flag_published = 1 {$temp_condition} ORDER BY item_order ASC", 'item_token');
...
$mmrpg_database_items_numbers = $this_database->get_array_list("SELECT item_token, (@item_row_number:=@item_row_number + 1) AS item_key FROM mmrpg_index_items WHERE item_flag_published = 1 {$temp_condition_unfiltered} ORDER BY item_flag_hidden ASC, item_order ASC;", 'item_token');
