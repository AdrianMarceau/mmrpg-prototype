...
function mmrpg_prototype_calculate_battle_points($update_session = false, $_GAME = false){
    ...
    if (!empty($_GAME['values']['battle_complete'])){
        foreach ($_GAME['values']['battle_complete'] AS $player_token => $player_battles){
            $total_battle_points += mmrpg_prototype_calculate_player_points($player_token, $update_session, $_GAME);
        }
    }
    ...
    if ($update_session){
        $_GAME['counters']['battle_points'] = $total_battle_points;
    }
...
function mmrpg_prototype_calculate_player_points($player_token, $update_session = false, $_GAME = false){
    ...
    if ($update_session){
        $_GAME['values']['battle_rewards'][$player_token]['player_points'] = $player_battle_points;
    }
...
function mmrpg_prototype_options_markup(&$battle_options, $player_token){
    ...
    foreach ($battle_options_reversed AS $this_key => $this_info){
        ...
        // Otherwise, if this is a normal battle option
            ...
            if (!empty($this_option_complete) || !empty($this_option_failure)){
                $this_option_title .= ' <hr />&laquo; Battle Records &raquo;';
                if (!empty($this_option_complete)){
                    if (!empty($this_option_complete['battle_points_earned'])){
                        $points_value = !empty($this_option_complete['battle_points_earned']) ? number_format($this_option_complete['battle_points_earned']) : 0;
                        $points_unit = 'Point'.(!empty($this_option_complete['battle_points_earned']) && $this_option_complete['battle_points_earned'] == 1 ? '' : 's');
                        $this_option_title .= ' <br />High Score : '.$points_value.' '.$points_unit;
                    }
                }
