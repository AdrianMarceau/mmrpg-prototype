...
require(MMRPG_CONFIG_ROOTDIR.'prototype/prototype.vars.php');
...
if (!empty($_SESSION[$session_token]['DEMO'])){
  ...
  require(MMRPG_CONFIG_ROOTDIR.'prototype/prototype.demo_menu.php');
 ...
 */
  ...
  require(MMRPG_CONFIG_ROOTDIR.'prototype/prototype.omega.php');

  // Do not process any events while we're in javascript mode
  if (!defined('MMRPG_SCRIPT_REQUEST')){

    // Include the demo mode options and markup
    require(MMRPG_CONFIG_ROOTDIR.'prototype/prototype.unlocks.php');
  ...
  if (!empty($_SESSION[rpg_game::session_token()]['DEMO'])){
    ...
    if (empty($this_data_condition) || in_array('this_player_token=dr-light', $this_data_condition)){
      ...
      if ($unlock_flag_light){
        require(MMRPG_CONFIG_ROOTDIR.'prototype/prototype.dr-light_menu.php');
      }
    }
  }
  // NORMAL MENU OPTIONS
    ...
    if (empty($this_data_condition) || $this_data_select == 'this_player_token' || in_array('this_player_token=dr-wily', $this_data_condition)){
      ...
      if ($unlock_flag_wily){
        require(MMRPG_CONFIG_ROOTDIR.'prototype/prototype.dr-wily_menu.php');
      }
    }
    ...
    if (empty($this_data_condition) || $this_data_select == 'this_player_token' || in_array('this_player_token=dr-cossack', $this_data_condition)){
      ...
      if ($unlock_flag_cossack){
        require(MMRPG_CONFIG_ROOTDIR.'prototype/prototype.dr-cossack_menu.php');
      }
    }
  }
