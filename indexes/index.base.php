...
if (!defined('MMRPG_CRITICAL_ERROR')){
    ...
    $temp_new_threads = $this_database->get_array_list("SELECT category_id, CONCAT(thread_id, '_', thread_mod_date) AS thread_session_token FROM mmrpg_threads WHERE thread_locked = 0 AND (thread_target = 0 OR thread_target = {$this_userinfo['user_id']} OR user_id = {$this_userinfo['user_id']}) AND thread_mod_date > {$temp_last_login}".($this_userid != MMRPG_SETTINGS_GUEST_ID ? "  AND thread_mod_user <> {$this_userid}" : ''));
    ...
    $temp_viewing_community = rpg_website::sessions_active('community/', 3, true);
...
<body id="mmrpg" class="index <?= !empty($temp_window_flag) ? 'windowFlag_'.$temp_window_flag : '' ?> type <?= MMRPG_SETTINGS_CURRENT_FIELDTYPE ?>">
<?/*
...
<div id="fb-root"></div>
    <div id="window" style="position: relative; height: auto !important;">
        <div class="banner">
            ...
            <? if (!empty($this_markup_header)): ?>
                <div class="sprite credits banner_credits" style="background-image: url(images/menus/menu-banner_credits.png?<?= MMRPG_CONFIG_CACHE_DATE?>);">Mega Man RPG Prototype</div>
            <? else: ?>
                <h1 class="sprite credits banner_credits" style="background-image: url(images/menus/menu-banner_credits.png?<?= MMRPG_CONFIG_CACHE_DATE?>);">Mega Man RPG Prototype</h1>
            <? endif; ?>
        ...
        <div class="menu field_type field_type_<?= MMRPG_SETTINGS_CURRENT_FIELDTYPE ?>">
            <? if(!defined('MMRPG_CRITICAL_ERROR')): ?>
                <div class="main">
                    ...
                    <a data-href="<?= MMRPG_CONFIG_ROOTURL ?>prototype/" target="_blank" class="link <?= $this_current_page == 'prototype' ? 'link_active field_type_empty' : '' ?>"><span><del>Play the Prototype</del></span></a>
        ...
        <?php if (!strstr(MMRPG_CONFIG_ROOTDIR, ':') && !MMRPG_CONFIG_IS_LIVE){ ?>
    ...
    <? endif; ?>
    <? if ($this_current_page == 'admin'): ?>
        <script type="text/javascript" src="scripts/jquery.scrollbar.min.js?<?= MMRPG_CONFIG_CACHE_DATE?>"></script>
...
if (rpg_game::is_user()){
    ...
    $temp_result = $this_database->query($temp_query);
}
?>