...
while ($this_formaction == 'post'){
    ...
    if (isset($update_array) || $formdata['post_target'] != 0){
        ...
        $temp_redirect_url .= '#post-'.$formdata['post_id'];
        //exit('$temp_redirect_url = '.$temp_redirect_url);
...
while ($this_formaction == 'thread'){
    ...
    elseif (strlen($formdata['thread_body']) > MMRPG_SETTINGS_DISCUSSION_MAXLENGTH && !COMMUNITY_VIEW_MODERATOR){
