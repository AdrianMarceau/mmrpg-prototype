...
$this_seo_title = (!empty($_REQUEST['thread_id']) ? 'Edit' : 'New').' Discussion | '.$this_seo_title;
...
if (!empty($this_current_target)){
    ...
    $target_user_info = $db->get_array("SELECT {$temp_user_fields} FROM mmrpg_users WHERE user_name_clean = '{$temp_user_name}' LIMIT 1");
} elseif (!empty($this_thread_info['thread_target'])){
    $temp_user_id = (int)($this_thread_info['thread_target']);
    $temp_user_fields = rpg_user::get_index_fields(true);
    $target_user_info = $db->get_array("SELECT {$temp_user_fields} FROM mmrpg_users WHERE user_id = {$temp_user_id} LIMIT 1");
}
...
// Compesate for missing thread title
if (empty($temp_thread_name)){
    $temp_thread_name = (!empty($_REQUEST['thread_id']) ? 'Edit ' : 'New ');
    $temp_thread_name .= ($this_category_info['category_id'] != 0 ? 'Discussion' : 'Message');
    if (!empty($target_user_info)){
        $temp_thread_name .= ' to '.(!empty($target_user_info['user_name_public']) ? $target_user_info['user_name_public'] : $target_user_info['user_name']);
    }
}
