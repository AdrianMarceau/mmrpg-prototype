...
if (!empty($this_current_filter)){ $this_seo_title = str_replace('Bosses | ', ('Bosses '.(!empty($this_current_filter) ? '('.$this_current_filter_name.' Type) ' : '').' | '), $this_seo_title); }
...
// If a specific boss has NOT been defined, show the quick-switcher
    ...
    // If we're not on a specific boss page, let's show global boss records
    if (empty($this_current_token)){
        echo(mmrpg_get_robot_database_records_markup('boss', 5).PHP_EOL);
