...
// -- PROTOTYPE VOID CAULDRON FUNCTIONALITY -- //

// "Mega Man RPG Void Cauldron / Recipe Wizard"
// This widget lets you throw items into a void and they turn into a new mission
(function(){

    // Define a global namespace for the void cauldron / recipe wizard functionality
    var thisVoidCauldron = function(){};
    var voidCauldronConfig = {};
    window.mmrpgVoidCauldron = thisVoidCauldron;
    ...
    voidCauldronConfig.maxItems = 10;
    voidCauldronConfig.maxTargets = 8;
    voidCauldronConfig.maxArrows = 5;
    voidCauldronConfig.maxLevel = 100;
    voidCauldronConfig.maxForte = 10;
    voidCauldronConfig.realMaxLevel = 999;
    voidCauldronConfig.realMaxForte = 99;
    voidCauldronConfig.currentItemMix = '';
    voidCauldronConfig.minQuantaPerClass = {mecha: 25, master: 100, boss: 500};
    voidCauldronConfig.voidPowersRequired = ['delta', 'spread', 'quanta', 'level', 'forte'];

    // Define a function for initializing the void cauldron's recipe wizard
    thisVoidCauldron.init = function($container, customData){
        console.log('%c' + 'thisVoidCauldron.init()', 'color: magenta;');
        ...
        _self.refreshUI();
        _self.done();
        //console.log('thisVoidCauldron is ' + ('%c' + 'ready'), 'color: lime;');
        //console.log('=> thisVoidCauldron:', _self);
        // end of thisVoidCauldron.init()
        };

    // Reset the void cauldron to its default state
    thisVoidCauldron.reset = function(refresh){
        console.log('%c' + 'thisVoidCauldron.reset()', 'color: magenta;');
        ...
        _self.refreshUI();
        _self.refreshHash();
        // end of thisVoidCauldron.reset()
        };

    // Setup the void cauldron with the provided data and container
    thisVoidCauldron.setup = function($container, customData){
        console.log('%c' + 'thisVoidCauldron.setup()', 'color: magenta;');
        //console.log('-> w/ $container:', typeof $container, $container.length, $container);
        //console.log('-> w/ customData:', typeof customData, customData);
        ...
        // end of thisVoidCauldron.setup()
        };

    // Trigger the done state to finalize the void cauldron's setup
    thisVoidCauldron.done = function(){
        console.log('%c' + 'thisVoidCauldron.done()', 'color: magenta;');
        const _self = thisVoidCauldron;
        _self.nowLoading(false);
        _self.nowReady(true);
        // end of thisVoidCauldron.done()
        };

    // Catalogue the provided content indexes' data into useful reference formats
    thisVoidCauldron.catalogIndexes = function(){
        console.log('%c' + 'thisVoidCauldron.catalogIndexes()', 'color: magenta;');
        const _self = thisVoidCauldron;
        const config = _self.config;
        ...
        // end of thisVoidCauldron.catalogIndexes()
        };

    // Generate the markup for the recipe wizard given the provided container
    thisVoidCauldron.generateMarkup = function($container){
        console.log('%c' + 'thisVoidCauldron.generateMarkup()', 'color: magenta;');
        //console.log('-> w/ $container:', typeof $container, $container.length, $container);
        const _self = thisVoidCauldron;
        const config = _self.config;
        const indexes = _self.indexes;
        ...
        // end of thisVoidCauldron.generateMarkup()
        };

    // Generate the item button markup to be used in the pallette area
    thisVoidCauldron.getItemButtonMarkup = function(){
        console.log('%c' + 'thisVoidCauldron.getItemButtonMarkup()', 'color: magenta;');
        const _self = thisVoidCauldron;
        const config = _self.config;
        const indexes = _self.indexes;
        const cache = _self.cache;
        ...
        // end of thisVoidCauldron.getItemButtonMarkup()
        };

    // Return a PowerRenderer sub-class that can be used to easily update the UI
    thisVoidCauldron.getPowerRenderer = function(){
        console.log('%c' + 'thisVoidCauldron.getPowerRenderer()', 'color: magenta;');
        const _self = thisVoidCauldron;
        const config = _self.config;
        return {
        ...
        // end of thisVoidCauldron.getPowerRenderer()
        };

    // Bind events to the various buttons and UI elements of the wizard
    thisVoidCauldron.bindEvents = function(){
        console.log('%c' + 'thisVoidCauldron.bindEvents()', 'color: magenta;');
        const _self = thisVoidCauldron;
        const config = _self.config;
        ...
        // end of thisVoidCauldron.bindEvents()
        };

    // Add an item to the mix, optionally refreshing the UI afterwards
    thisVoidCauldron.addItem = function(item, refresh){
        console.log('%c' + 'thisVoidCauldron.addItem() w/ ' + item.token, 'color: magenta;');
        ...
        _self.refreshUI();
        _self.refreshHash();
        // end of thisVoidCauldron.addItem()
        };

    // Remove an item from the mix, optionally refreshing the UI afterwards
    thisVoidCauldron.removeItem = function(item, refresh){
        console.log('%c' + 'thisVoidCauldron.removeItem() w/ ' + item.token, 'color: magenta;');
        //console.log('-> w/ item:', item);
        const _self = thisVoidCauldron;
        var token = item.token;
        var quantity = item.quantity || 1;
        ...
        _self.refreshUI();
        _self.refreshHash();
        // end of thisVoidCauldron.removeItem()
        };

    // Parse an added item and apply it's values and/or intrinsic properties to existing void powers
    thisVoidCauldron.parseItem = function(item, quantity, powers){
        console.log('%c' + 'thisVoidCauldron.parseItem() w/ ' + item.token + ' x' + quantity, 'color: magenta;');
        ...
        */
        ...
        // end of thisVoidCauldron.parseItem()
        };

    // Parse the entire item mix, one-by-one and in-order, applying their values/effects to the void powers
    thisVoidCauldron.parseItemMix = function(mix){
        console.log('%c' + 'thisVoidCauldron.parseItemMix()', 'color: magenta;');
        ...
        // end of thisVoidCauldron.parseItemMix()
        };

    // Filter the void powers for only ones that represent stats
    thisVoidCauldron.filterStatPowers = function(powers, sort){
        sort = typeof sort === 'undefined' ? true : sort;
        console.log('%c' + 'thisVoidCauldron.filterStatPowers()', 'color: magenta;');
        ...
        // end of thisVoidCauldron.filterStatPowers()
        };

    // Filter the void powers for only ones that represent types
    thisVoidCauldron.filterTypePowers = function(powers, sort){
        sort = typeof sort === 'undefined' ? true : sort;
        console.log('%c' + 'thisVoidCauldron.filterTypePowers()', 'color: magenta;');
        //console.log('-> w/ powers:', powers, 'sort:', sort);
        ...
        // end of thisVoidCauldron.filterTypePowers()
        };

    // Filter the void flows for only ones that represent types
    thisVoidCauldron.filterFlowPowers = function(flows, sort){
        sort = typeof sort === 'undefined' ? true : sort;
        console.log('%c' + 'thisVoidCauldron.filterFlowPowers()', 'color: magenta;');
        ...
        // end of thisVoidCauldron.filterFlowPowers()
        };

    // Calculate the void powers by looping through items in the mix and parsing their values/effects
    thisVoidCauldron.calculatePowers = function(){
        console.log('%c' + 'thisVoidCauldron.calculatePowers()', 'color: magenta;');
        ...
        // end of thisVoidCauldron.calculatePowers()
        };

    // Generate a target queue given all the void powers and other contextual data we've calculated
    thisVoidCauldron.generateTargetQueue = function(robots, types, stats){
        console.log('%c' + 'thisVoidCauldron.generateTargetQueue()', 'color: magenta;');
        ...
        return targetQueue;
        // end of thisVoidCauldron.generateTargetQueue()
        };

    // Generate a new mission using the void powers and other contextual data we've calculated
    thisVoidCauldron.generateMission = function(){
        console.log('%c' + 'thisVoidCauldron.generateMission()', 'color: magenta;');
        ...
        // end of thisVoidCauldron.generateMission()
        };

    // Refresh the UI with the calculated void powers, generated mission, and any other relevant data
    thisVoidCauldron.refreshUI = function(){
        console.log('%c' + 'thisVoidCauldron.refreshUI()', 'color: magenta;');
        ...
        // end of thisVoidCauldron.refreshUI()
        };

    // Get the mix string for the current selection of items added to the void cauldron
    thisVoidCauldron.getMixString = function(){
        console.log('%c' + 'thisVoidCauldron.getMixString()', 'color: magenta;');
        ...
        // end of thisVoidCauldron.getMixString()
        };

    // Set the current status of the void cauldron to a given value
    thisVoidCauldron.setStatus = function(status, value, prefix){
        console.log('%c' + 'thisVoidCauldron.setStatus()', 'color: magenta;');
        const _self = thisVoidCauldron;
        const config = _self.config;
        ...
        // end of thisVoidCauldron.setStatus()
        };

    // Update "ready" status (true if void cauldron currently ready to generate a new mission)
    thisVoidCauldron.nowReady = function(value){
        console.log('%c' + 'thisVoidCauldron.nowReady()', 'color: magenta;');
        const _self = thisVoidCauldron; return _self.setStatus('ready', value, 'is');
        // end of thisVoidCauldron.nowReady()
        };

    // Update "loading" status (true if void cauldron currently loading data)
    thisVoidCauldron.nowLoading = function(value){
        console.log('%c' + 'thisVoidCauldron.nowLoading()', 'color: magenta;');
        const _self = thisVoidCauldron; return _self.setStatus('loading', value, 'is');
        // end of thisVoidCauldron.nowLoading()
        };

    // Update "content" status (true if void cauldron currently has content to display)
    thisVoidCauldron.hasContent = function(value){
        console.log('%c' + 'thisVoidCauldron.hasContent()', 'color: magenta;');
        const _self = thisVoidCauldron; return _self.setStatus('content', value, 'has');
        // end of thisVoidCauldron.hasContent()
        };

    // Sort a list of tokens by their strings' first-appearance in the item mix string (if at all)
    thisVoidCauldron.sortTokensByItemOrder = function(unsortedTokens){
        console.log('%c' + 'thisVoidCauldron.sortTokensByItemOrder()', 'color: magenta;');
        ...
        return sortedTokens;
        // end of thisVoidCauldron.sortTokensByItemOrder()
        };

    // Get the ordinal suffix (st, nd, rd from 1st, 2nd, 3rd) for a given number
    thisVoidCauldron.getOrdinalSuffix = function(num){
        //console.log('%c' + 'thisVoidCauldron.getOrdinalSuffix()', 'color: magenta;');
        const suffixes = ["th", "st", "nd", "rd"];
        const value = num % 100;
        return suffixes[(value - 20) % 10] || suffixes[value] || suffixes[0];
        // end of thisVoidCauldron.getOrdinalSuffix()
        };

    // Get the current hash parameters from the URL and return them as an object
    thisVoidCauldron.getHashParams = function(){
    ...
    // Refresh the URL hash with the current mix string, assuming their is one, or clear it if not
    thisVoidCauldron.refreshHash = function(){
        console.log('%c' + 'thisVoidCauldron.refreshHash()', 'color: magenta;');
        ...
        // end of thisVoidCauldron.refreshHash()
        };

    // Show the debug information for the current void cauldron instance in the console log
    thisVoidCauldron.showDebug = function(kind){
        console.log('%c' + 'thisVoidCauldron.showDebug()', 'color: magenta;');
        kind = kind || 'all';
        const _self = thisVoidCauldron;
        ...
        // end of thisVoidCauldron.showDebug()
        };
