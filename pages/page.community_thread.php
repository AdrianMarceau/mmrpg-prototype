...
<div class="posts_body">
  ...
  if (!empty($this_posts_count)){
    if ($this_category_info['category_id'] != 0){
      ?>
      <h2 id="comment-listing" class="subheader thread_posts_count field_type_<?= !empty($this_thread_info['thread_colour']) ? $this_thread_info['thread_colour'] : MMRPG_SETTINGS_CURRENT_FIELDTYPE ?>"><?= $this_posts_count == 1 ? '1 Comment' : $this_posts_count.' Comments' ?></h2>
      <?
    } else {
      ?>
      <h2 id="comment-listing" class="subheader thread_posts_count field_type_<?= !empty($this_thread_info['thread_colour']) ? $this_thread_info['thread_colour'] : MMRPG_SETTINGS_CURRENT_FIELDTYPE ?>"><?= $this_posts_count == 1 ? '1 Reply' : $this_posts_count.' Replies' ?></h2>
      <?
    }
