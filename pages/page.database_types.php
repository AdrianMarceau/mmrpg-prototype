...
<div id="types">
  ...
  <div class="subbody">
    ...
    <div style="overflow: hidden; padding-top: 10px;">
      ...
      <?php foreach($mmrpg_database_robots_types AS $type_mechanic => $type_array): ?>
        <?php $this_overall_count = $type_mechanic == 'cores' ? $mmrpg_database_types_count : $mmrpg_database_types_count - 1;  ?>
        <div class="type_chart type_chart_cores">
          ...
          if ($type_mechanic == 'cores'){
            ...
            <div class="text wrapper">
              ...
              <table width="100%">
                <colgroup>
                  <?php foreach($temp_mechanic_data AS $type_token => $type_count): ?>
                  <?php if (empty($type_count) || $type_token == 'none'){ continue; } ?>
                  <?php $this_percent = round(($type_count / $mmrpg_database_robots_count) * 100, 1);  ?>
                ...
                <tbody>
                  <tr>
                    ...
                    <?php if (empty($type_count) || $type_token == 'none'){ continue; } ?>
                    <?php $this_percent = round(($type_count / $mmrpg_database_robots_count) * 100, 1);  ?>
                    <?php $this_title = ucfirst($type_token).' Type | '.$type_count.' / '.$mmrpg_database_robots_count.' Robots | '.$this_percent.'% ';  ?>
          ...
          // ROBOT WEAKNESSES / RESISTANCES / AFFINITIES / IMMUNITIES
            ...
            <div class="text wrapper">
              ...
              <table width="100%">
                <colgroup>
                  <?php foreach($temp_mechanic_data AS $type_token => $type_count): ?>
                  <?php if (empty($type_count)){ continue; } ?>
                  <?php $this_percent = round(($type_count / $temp_mechanic_count) * 100, 1);  ?>
                  <col width="<?= $this_percent.'%' ?>">
                  <?php endforeach; ?>
                </colgroup>
                <tbody>
        ...
        <div class="wrapper">
          ...
          <table width="100%">
            <colgroup>
              <?php foreach($temp_mechanic_data AS $type_token => $type_count): ?>
              <?php if (empty($type_count) || $type_token == 'none'){ continue; } ?>
              <?php $this_percent = round(($type_count / $mmrpg_database_abilities_count) * 100, 1);  ?>
              <col width="<?= $this_percent.'%' ?>">
              <?php endforeach; ?>
            </colgroup>
            <tbody>
              <tr>
                ...
                <?php if (empty($type_count) || $type_token == 'none'){ continue; } ?>
                <?php $this_percent = round(($type_count / $mmrpg_database_abilities_count) * 100, 1);  ?>
                <?php $this_title = ucfirst($type_token).' Type | '.$type_count.' / '.$mmrpg_database_abilities_count.' Abilities | '.$this_percent.'% ';  ?>
                <td><div class="type_percent ability_type ability_type_<?= $type_token ?>" title="<?= $this_title ?>"><?= $this_percent.'%' ?></div><div class="type_label" title="<?= $this_title ?>"><?= ucfirst($type_token)?></div></td>
                <?php $key_counter++; ?>
                <?php endforeach; ?>
              </tr>
